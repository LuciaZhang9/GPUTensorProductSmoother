...
private:
  ...
  void
  step_impl(VectorType        &dst,
            VectorType const  &src,
            const unsigned int step_start = 0) const
  {
    ...
    // *** compute approximate solutions: x^{k},
    // k=step_start,...,#smoothing_steps
    for (unsigned int k = step_start;
         k < additional_data.number_of_smoothing_steps;
         ++k)
      {
        ...
        if (transpose)
          {
            AssertThrow(
              false, ExcMessage("TODO preconditioner->Tvmult(tmp, residual)"));
          }
        else
