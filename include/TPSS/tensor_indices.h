...
namespace Tensors
{
   ...
   * order. That is the first index of the multi-index runs faster than second
   * and so on.
   *
   * order : the order of the multi-index
  ...
  template <int order, typename IntType = unsigned int>
  std::vector<IntType>
  index_fibre(const std::array<IntType, order - 1> index,
              const int                            mode,
              const IntType                        size)
  {
    ...
    std::generate(multiindex.begin(),
                  multiindex.end(),
                  [&, mode, k = int{0}]() mutable {
                    return (k++ != mode) ? *(elem++) : 0U;
                  });
    for (IntType i = 0; i < size; ++i)
      {
        multiindex[mode] = i;
        // std::cout << Tensors::multiindex_to_string<order>(multiindex) <<
        // std::endl;
  ...
  struct TensorHelper
  {
     ...
     * edge numbers is empty. If the index is located at a vertex dim edge
     ...
     */
    unsigned int
    get_edge_no_1d(const std::array<IntType, order> &multi_index,
    ...
    bool
    is_edge_index_1d(const std::array<IntType, order> &multi_index,
                     const unsigned int                mode) const
    {
      AssertIndexRange(mode, order);
      return is_first_index_1d(multi_index, mode) ||
             is_last_index_1d(multi_index, mode);
    }
