...
namespace TPSS
{
  ...
  get_dof_layout(const FiniteElement<dim, dim> &finite_element)
  {
    ...
    else if (finite_element.get_name().find("RaviartThoma") !=
             std::string::npos)
              ...
              const unsigned int fe_degree)
    {
      if (dof_layout == DoFLayout::DGQ)
        ...
        if (patch_variant == TPSS::PatchVariant::vertex)
          return n_cells_per_direction(TPSS::PatchVariant::vertex) *
                   (fe_degree + 1) -
                 3;
      return -1;
    }
  ...
  /**
   * Internal assembler for 1D dof-related matrices exploiting the tensor
   * product form of the shape functions
   */
  ...
  public:
             ...
             const dealii::ArrayView<Number>       &matrix)
    {
      ...
      apply_impl<add>(ansatz_data.data(),
                      test_data.data(),
                      dx.data(),
                      matrix.data());
    }
                 ...
                 const dealii::ArrayView<Number>       &matrix_out)
    {
      ...
      scale_array2dim<fe_order, fe_order, false>(&factor,
                                                 matrix_in.data(),
                                                 matrix_out.data());
    }
  ...
  private:
    ...
    static void
    scale_array2dim(const Number *factor,
                    const Number *array_in,
                    Number       *array_out)
    {
      ...
      for (int r = 0; r < n_rows; ++r)
        for (int c = 0; c < n_cols; ++c)
          array_out[r * stride + c] =
            array_in[r * stride + c] * factor[variable ? c : 0];
    }
        ...
        for (int i = 0; i < fe_order; ++i)
          {
            ...
            for (int q = 0; q < n_q_points; ++q)
              m_ji +=
                ansatz_data[i * stride + q] * test_data_dx[j * stride + q];
            if (add)
              *matrix += m_ji;
            else
              *matrix = m_ji;
                         ...
                         number>
  {
    static constexpr unsigned int dofs_per_cell =
      dealii::Utilities::pow(fe_order, dim);
      ...
      , quad_weights(quad_weights_.begin())
    {
      ...
      // TODO how do we handle the face assembly case, where we use
      // n_q_points_1d == 1 ?
    ...
    template <bool add>
    static void
    apply(const number *ansatz_data,
          const number *test_data,
          const number *dx,
          number       *matrix);
