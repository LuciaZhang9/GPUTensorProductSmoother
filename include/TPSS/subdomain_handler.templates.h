...
void
SubdomainHandler<dim, Number>::reinit(
  const dealii::MatrixFree<dim, Number> *mf_storage_in,
  const AdditionalData                   additional_data_in)
{
  clear();
...
SubdomainHandler<dim, Number>::internal_reinit()
{
  ...
  Assert(additional_data.level != static_cast<unsigned int>(-1),
         ExcNotImplemented());
  Assert(additional_data.patch_variant != TPSS::PatchVariant::invalid,
         ExcInvalidState());
  Assert(additional_data.smoother_variant != TPSS::SmootherVariant::invalid,
         ExcInvalidState());
  ...
  std::vector<const dealii::DoFHandler<dim> *> unique_dof_handlers;
  {
    const auto check_dof_handler = [&](const auto dof_handler) {
      ...
      AssertIndexRange(additional_data.level,
                       dof_handler->get_triangulation().n_global_levels());
      Assert(dof_handler->get_fe().is_primitive(),
             ExcMessage(
               "Currently, only primitive finite elements are supported"));
      Assert(
      ...
      Assert(dof_handler->get_fe().n_blocks() == 1 ||
               dof_handler->get_fe().n_blocks() ==
                 dof_handler->get_fe().n_components(),
             ExcMessage(
               "Currently, nested block structures are not supported."));
    ...
    for (auto dofh_index = 1U; dofh_index < n_dof_handlers; ++dofh_index)
      {
        ...
        const auto it =
          std::find_if(unique_dof_handlers.begin(),
                       unique_dof_handlers.end(),
                       [&](const auto unique_dofh) {
                         return dofh.get_fe() == unique_dofh->get_fe();
                       });
        const unsigned int unique_dofh_index =
          std::distance(unique_dof_handlers.begin(), it);
  ...
  patch_info_data.patch_distribution_func =
    additional_data.patch_distribution_func;
  ...
  // *** Calling the constructor, partitions patches with respect to
  // vectorization.
  ...
  { // *** (partially) store dof indices and patch-local dof information
    ...
    dof_info_data.compute_ras_boolean_weights =
      additional_data.compute_ras_boolean_weights;
        ...
        if (!additional_data.foreach_dofh.empty())
          {
            ...
            dof_info_data.dirichlet_ids =
              additional_data.foreach_dofh[dofh_index].dirichlet_ids;
            dof_info_data.force_no_boundary_condition =
              additional_data.foreach_dofh[dofh_index]
                .force_no_boundary_condition;
          }
        auto &dof_info = unique_dof_infos[unique_dofh_index];
        const auto [dummy, not_initialized] =
          initialized_indices.insert(unique_dofh_index);
        (void)dummy;
        if (not_initialized)
...
template <int dim, typename Number>
unsigned int
SubdomainHandler<dim, Number>::guess_grain_size(
  const unsigned int n_subdomain_batches) const
{
  ...
  if (grain_size == 0)
    {
      ...
      const unsigned int dofs_per_cell =
        get_dof_handler(0).get_fe().dofs_per_cell;
