...
namespace LinAlg
{
  ...
  scaling(const AlignedVector<Number> &vec, const Number &scalar)
  {
    ...
    std::transform(vec.begin(),
                   vec.end(),
                   scaled_vec.begin(),
                   [&](const auto &elem) { return scalar * elem; });
    return scaled_vec;
  }
  ...
  template <typename Number>
  Number
  inner_product(const AlignedVector<Number> &lhs,
                const AlignedVector<Number> &rhs)
  {
    AssertDimension(lhs.size(), rhs.size());
   ...
   * scalar is nearly zero is scaled by zero instead of the actual inverse
   * scalar avoiding a division by zero.
   */
  template <typename Number>
  AlignedVector<Number>
                     ...
                     const std::array<std::size_t, 2> &column_range)
  {
    ...
    AssertIndexRange(fixed_col_or_row,
                     (transpose ? dst.size(0) : dst.size(1)) + 1U);
    ...
    for (std::size_t i = 0; i < m; ++i)
      for (std::size_t j = 0; j < n; ++j)
        {
          if (transpose)
            dst(fixed_col_or_row, i + j * m) =
              src(i + row_begin, j + column_begin);
          else
            dst(i + j * m, fixed_col_or_row) =
              src(i + row_begin, j + column_begin);
        }
  }
  ...
  vect_impl(const Table<2, Number> &M)
  {
    ...
    vect_of_block_impl<Number, transpose>(
      vect, 0U, M, {{0U, M.size(0)}}, {{0U, M.size(1)}});
    return vect;
  }
  ...
  {
    AssertIndexRange(fixed_column_or_row,
                     transpose_src ? src.size(0) : src.size(1));
    AssertDimension(transpose_src ? src.size(1) : src.size(0),
                    n_rows * n_columns);
    ...
    for (auto i = 0U; i < n_rows; ++i)
      for (auto j = 0U; j < n_columns; ++j)
        if (do_scaling)
          {
            matrix(i, j) = alpha * (transpose_src ?
                                      src(fixed_column_or_row, i + j * n_rows) :
        ...
        else
          {
            (void)alpha;
            matrix(i, j) =
              (transpose_src ? src(fixed_column_or_row, i + j * n_rows) :
                               src(i + j * n_rows, fixed_column_or_row));
          }
    return matrix;
  }
  ...
  {
    return folding_impl<Number, OtherNumber, true, true>(
      src, n_rows, n_columns, alpha, fixed_row);
  }
  ...
  Table<2, Number>
  folding(const Table<2, Number> &src,
          const unsigned int      n_rows,
          const unsigned int      n_columns)
  {
    AssertDimension(src.size(1), 1U);
    return folding_impl<Number, OtherNumber, false, false>(
      src, n_rows, n_columns, 1., 0U);
  }
  ...
  template <typename Number, typename OtherNumber = Number>
  Table<2, Number>
  foldingT(const Table<2, Number> &src,
           const unsigned int      n_rows,
           const unsigned int      n_columns)
  {
    AssertDimension(src.size(1), 1U);
    return folding_impl<Number, OtherNumber, false, true>(
      src, n_rows, n_columns, 1., 0U);
  }
   ...
   * consisting only of zeros and replaces inverse scalars by zero (all
   * remaining lanes are processed as usual).
   */
  template <typename Number>
  void
    ...
    for (auto j = 0U; j < n - 1; ++j)
      {
        ...
        sadd(r_n,
             -inverse_scalar_if(inner_product(r_j, r_j)) *
               inner_product(r_n, r_j),
             r_j);
      }
  }
    ...
    for (auto lane = 0U; lane < get_macro_size<Number>(); ++lane)
      {
        const auto &eigenvalues =
          compute_eigenvalues(table_to_fullmatrix(matrix, lane));
        Assert(std::all_of(eigenvalues.cbegin(),
                           eigenvalues.cend(),
                           [](const auto &val) {
                             return has_nearly_zero_abs(val.imag());
                           }),
  ...
  Table<2, Number>
  scaled_sum(const Number2          &alpha,
             const Table<2, Number> &lhs,
             const Table<2, Number> &rhs)
  {
    return sum_impl<Number, Number2, false, true, false>(
      lhs, rhs, 1., alpha, 1.);
  }
  ...
  template <typename Number, typename Number2 = Number>
  Table<2, Number>
  ssum(const Number2          &alpha,
       const Table<2, Number> &lhs,
       const Table<2, Number> &rhs)
  {
    return sum_impl<Number, Number2, true, false, false>(
      lhs, rhs, alpha, 1., 1.);
  }
  ...
  template <typename Number, typename Number2 = Number>
  Table<2, Number>
  sums(const Table<2, Number> &lhs,
       const Number2          &beta,
       const Table<2, Number> &rhs)
  {
    return sum_impl<Number, Number2, false, false, true>(
      lhs, rhs, 1., 1., beta);
  }
  ...
  ssums(const Number2          &alpha,
        ...
        const Table<2, Number> &rhs)
  {
    return sum_impl<Number, Number2, true, false, true>(
      lhs, rhs, alpha, 1., beta);
  }
      ...
      for (auto j = 0U; j < rn; ++j)
        {
          Number inner_prod(0.);
          for (auto k = 0U; k < n; ++k)
            inner_prod += (transpose_lhs ? lhs(k, i) : lhs(i, k)) *
                          (transpose_rhs ? rhs(j, k) : rhs(k, j));
  ...
  compute_inverse_svd(const Table<2, Number> &matrix)
  {
    ...
    for (auto lane = 0U; lane < get_macro_size<Number>(); ++lane)
      {
        LAPACKFullMatrix<scalar_value_type> micro_matrix(matrix.size(0),
                                                         matrix.size(1));
        micro_matrix = table_to_fullmatrix(matrix, lane);

        const auto &[tildeV, micro_invSigma, tildeUT] =
          ::compute_inverse_svd(micro_matrix);
        ...
        for (auto i = 0U;
             i < std::min<unsigned int>(invSigma.size(0), invSigma.size(1));
             ++i)
  ...
  /**
   * Computes the matrix-vector multiplication w = A^T * v, where A^T denotes
   * the transpose of matrix A, returning the column vector w.
   */
  template <typename Number>
  AlignedVector<Number>
  ...
  /**
   * Computes the matrix-vector multiplication w = v^T * A, where v^T denotes
   * the transpose of column vector v, returning the row vector w.
   */
  template <typename Number>
  AlignedVector<Number>
   ...
   * the transpose of column vector v and A^T the transpose of matrix A,
   * returning the row vector w.
   */
  template <typename Number>
  AlignedVector<Number>
...
// Calculate the unfolding matrix in some direction of a third order tensor with
// polyadic rank one, that is a tensor given as the polyadic product of three
// vectors
template <typename Number>
Table<2, Number>
unfold_rank1(std::array<AlignedVector<Number>, 3> polyadic_factors,
...
// Calculate the unfolding matrix in some direction of a third order tensor with
// higher polyadic rank, that is a tensor given as the sum of polyadic products
// of three vectors, here each polyadic product is given as one entry of the
// vector polyadic_factors
template <typename Number>
Table<2, Number>
