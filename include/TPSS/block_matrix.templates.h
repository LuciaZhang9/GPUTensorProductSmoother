...
namespace Tensors
{
  ...
  SchurComplementBase<MatrixType, Number>::SchurComplementBase(
  ...
  template <typename MatrixType, typename Number>
  void
  SchurComplementBase<MatrixType, Number>::vmult(
  ...
  template <typename MatrixType, typename Number>
  SchurComplementBasic<MatrixType, Number>::SchurComplementBasic(
  ...
  template <typename MatrixType, typename Number>
  void
  SchurComplementBasic<MatrixType, Number>::invert(
    ...
    if (additional_data.kronecker_rank != numbers::invalid_unsigned_int)
      {
        ...
        std::vector<tensor_type> low_rank_representation(
          additional_data.kronecker_rank);
        ...
        compute_ksvd(Base::as_table(),
                     low_rank_representation,
                     additional_data.lanczos_iterations);

        TensorProductMatrix<2, Number> low_rank_matrix(low_rank_representation);
        Sinv =
          std::make_shared<InverseTable<Number>>(low_rank_matrix.as_table(),
      ...
      {
        Sinv =
          std::make_shared<InverseTable<Number>>(Base::as_table(),
                                                 additional_data.basic_inverse);
      }
  }
  ...
  BlockGaussianInverseBase<MatrixType, SchurType, Number>::
  ...
  BlockMatrixBase<MatrixType, Number>::operator=(
    const BlockMatrixBase<MatrixType, Number> &other)
  {
  ...
  template <typename MatrixType, typename Number>
  const MatrixType &
  BlockMatrixBase<MatrixType, Number>::get_block(
  ...
  {
    Assert(has_consistent_block_rows(),
           ExcMessage("Inconsistent block structure."));
    std::size_t n_rows = 0;
  ...
  {
    Assert(has_consistent_block_columns(),
           ExcMessage("Inconsistent block structure."));
    std::size_t n_cols = 0;
  ...
  template <typename MatrixType, typename Number>
  void
  BlockMatrixBase<MatrixType, Number>::vmult(
  ...
  template <typename MatrixType, typename Number>
  std::size_t
  BlockMatrixBase<MatrixType, Number>::block_index(
    const std::size_t row_index,
    const std::size_t col_index) const
  {
  ...
  void
  BlockMatrixBase<MatrixType, Number>::blockwise_action(
    ...
    const ArrayView<const Number> &src) const
  {
    ...
    //        ExcMessage("TODO BlockMatrixBase is not quadratic.")); //
    //        quadratic !
    // Assert(n_rows_1d == -1 || src.size() % n_rows_1d == 0,
    //        ExcMessage("Input vector sizes are not a multiple of the static
    //        size."));
  ...
  BlockMatrixBasic<MatrixType, Number>::operator=(
    const BlockMatrixBasic<MatrixType, Number> &other)
  {
    Base::operator=(other);
  ...
  {
    basic_inverse =
      std::make_shared<InverseTable<Number>>(this->as_table(), additional_data);
  }
  ...
  template <typename MatrixType, typename Number>
  void
  BlockMatrixBasic<MatrixType, Number>::apply_inverse(
    const ArrayView<Number>       &dst,
    const ArrayView<const Number> &src) const
  {
    AssertThrow(
      basic_inverse,
      ExcMessage(
        "Have you called invert() after finishing the submission of entries?"));
  ...
  template <typename MatrixType, typename Number>
  MatrixType &
  BlockMatrixBasic2x2<MatrixType, Number>::get_block(
    const std::size_t row_index,
    const std::size_t col_index)
  {
    /// modification of any blocks results in an invalid inverse
    ...
    const bool            omit_inversion_of_blocks)
  {
    ...
    basic_inverse =
      std::make_shared<BlockGaussianInverseBasic<MatrixType, Number>>(
        A, B, C, D, additional_data);
  }
  ...
  template <typename MatrixType, typename Number>
  void
  BlockMatrixBasic2x2<MatrixType, Number>::apply_inverse(
    const ArrayView<Number>       &dst,
    const ArrayView<const Number> &src) const
  {
    AssertThrow(
