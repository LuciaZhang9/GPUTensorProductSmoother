...
struct ZeroThresholdImpl
{
  static_assert(std::is_floating_point_v<Number>,
                "Number is no floating point type.");

  static const Number value;
};

template <typename Number>
const Number ZeroThresholdImpl<Number>::value =
...
const VectorizedArray<Number>
  ZeroThresholdImpl<VectorizedArray<Number>>::value =
 ...
 */
template <typename Number,
          typename scalar_value_type = typename ExtractScalarType<Number>::type>
bool
  ...
  const scalar_value_type &threshold = zero_threshold<scalar_value_type>)
{
  for (auto lane = 0U; lane < get_macro_size<Number>(); ++lane)
    {
      const bool is_nearly_zero =
        std::abs(scalar_value(value, lane)) < threshold;
      if (!is_nearly_zero)
        return false;
    }
  return true;
...
less_than(const VectorizedArray<Number> &lhs,
...
template <typename Number>
FullMatrix<Number>
table_to_fullmatrix(const Table<2, VectorizedArray<Number>> &table,
                    const unsigned int                       lane = 0)
{
...
alignedvector_to_vector(const AlignedVector<Number> &avec,
                        const unsigned int           lane)
{
  std::vector<typename ExtractScalarType<Number>::type> vec;
  std::transform(avec.begin(),
                 avec.end(),
                 std::back_inserter(vec),
                 [lane](const auto &val) { return scalar_value(val, lane); });
  return vec;
}
