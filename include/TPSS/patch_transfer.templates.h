namespace TPSS
{
    ...
    for (unsigned int lane = 0; lane < macro_size; ++lane)
      {
        ...
        for (auto dst_value = dst.begin(); dst_value != dst.end();
             ++dof_index, ++dst_value)
  ...
  template <typename VectorType>
  void
  PatchTransfer<dim, Number>::gather_add(
    ...
    const VectorType                       &src) const
  {
    AssertDimension(dst.size(), n_dofs_per_patch());
    const auto dst_view =
      make_array_view<VectorizedArray<Number>>(dst.begin(), dst.end());
    ...
    const ArrayView<const VectorizedArray<Number>> src) const
  {
    ...
    for (unsigned int lane = 0;
         ...
         ++lane)
      {
        ...
        if (is_restricted)
          {
            const auto is_restricted_dof =
              patch_dof_worker.get_restricted_dof_flags(patch_id, lane);
            AssertDimension(is_restricted_dof.size(),
                            global_dof_indices.size());
            auto i = 0U;
            for (auto src_value = src.cbegin(); src_value != src.cend();
                 ++i, ++src_value)
              {
                ...
                if (!is_restricted_dof[i])
                  {
                    if (do_add)
                      internal::local_element(dst, dof_index) +=
                        (*src_value)[lane];
                    else
                      internal::local_element(dst, dof_index) =
                        (*src_value)[lane];
  ...
  template <typename VectorType>
  void
  PatchTransfer<dim, Number>::scatter_add(
    VectorType                                    &dst,
    const ArrayView<const VectorizedArray<Number>> src) const
  {
  ...
  {
    const auto src_view =
      make_array_view<const VectorizedArray<Number>>(src.begin(), src.end());
  ...
  template <typename VectorType>
  void
  PatchTransfer<dim, Number>::scatter(
    VectorType                                    &dst,
    const ArrayView<const VectorizedArray<Number>> src) const
  {
  ...
  template <typename VectorType>
  void
  PatchTransfer<dim, Number>::rscatter_add(
    VectorType                                    &dst,
    const ArrayView<const VectorizedArray<Number>> src) const
  {
  ...
  template <typename BlockVectorType>
  void
  PatchTransferBlock<dim, Number>::gather_add(
    AlignedVector<VectorizedArray<Number>> &dst,
