...
namespace TPSS
{
  ...
  public:
    ...
    void
    initialize(const dealii::DoFHandler<dim> *dof_handler,
    ...
    /**
     * An array that stores how many vectorization lanes of a macro patch
  ...
  struct PatchInfo<dim>::AdditionalData
  {
    ...
    std::function<std::vector<std::vector<PatchIterator>>(
      ...
      coloring_func;
    std::function<
      void(const DoFHandler<dim>                  *dof_handler,
           const AdditionalData                    additional_data,
    ...
    std::function<void(const DoFHandler<dim>   &dof_handler_in,
                       const std::vector<std::vector<typename TPSS::PatchInfo<
                         dim>::PatchIterator>> &colored_iterators,
                       const std::string)>
         visualize_coloring;
    bool use_tbb       = false;
  ...
  struct PatchInfo<dim>::PartitionData
  {
     ...
     */
    std::pair<unsigned int, unsigned int>
    get_patch_range(const unsigned int partition,
     ...
     * colors. Two consecutive integers of the inner vector represent a
     * half-open interval, so-called patch range.
     */
  ...
  struct PatchInfo<dim>::InternalData
  {
    ...
    /**
     * This flat array stores all cells required to build regular patches of
     * cells. The successive alignment of patches of cells is such that all
     * interior subdomains are stored first, then the ones at the physical
     * boundary. The array has following lexicographical ordering:
     *
  ...
  template <int dim>
  FaceInfoLocal<dim>::FaceInfoLocal(
    const unsigned int               my_cell_no,
    const std::vector<CellIterator> &cell_collection_in)
    : cell_number(my_cell_no)
    , n_cells(cell_collection_in.size())
  {
  ...
  template <int dim>
  inline constexpr unsigned int
  FaceInfoLocal<dim>::face_no(const unsigned int face_no_1d,
  ...
  FaceInfoLocal<dim>::at_lower_neighbor(const unsigned int face_no) const
  {
    ...
    const bool neighbor_cell_no_is_lower =
      face_to_cell_number[face_no] < cell_number;
  ...
  PatchInfo<dim>::get_cell_level_and_index(
    const unsigned int cell_position) const
  {
    AssertIndexRange(cell_position, n_cells_plain());
    AssertDimension(n_cells_plain(),
                    get_internal_data()->cell_level_and_index_pairs.size());
  ...
  PatchInfo<dim>::get_cell_iterator(const unsigned int cell_position) const
  {
    ...
    const auto [cell_level, cell_index] =
      get_cell_level_and_index(cell_position);
  ...
  template <int dim>
  PatchInfo<dim>::GhostPatch::GhostPatch(const unsigned int proc,
  ...
  template <int dim>
  inline void
  PatchInfo<dim>::GhostPatch::submit_id(const unsigned int proc,
                                        const CellId      &cell_id)
  {
  ...
  PatchInfo<dim>::GhostPatch::str() const
  {
    ...
    unsigned   i    = 0;
    for (auto key_value = proc_to_cell_ids.cbegin();
         key_value != proc_to_cell_ids.cend();
         ++key_value, ++i)
      oss << "(" << key_value->first << ", "
          << vector_to_string(key_value->second)
          << ((i + 1) < size ? "), " : ")}");
    return oss.str();
  }
  ...
  PatchInfo<dim>::PartitionData::n_subdomains() const
  {
    return partitions.empty() ?
             0 :
             (partitions.back().empty() ? 0 : partitions.back().back());
  }
                                                 ...
                                                 const unsigned int color) const
  {
    ...
    return std::make_pair(partitions[color][partition],
                          partitions[color][partition + 1]);
  }
  ...
  {
    const auto n_cells_plain_mpimax =
      Utilities::MPI::max(n_cells_plain(), MPI_COMM_WORLD);
