...
template <typename Number>
Vector<Number>
compute_eigenvalues_symm(LAPACKFullMatrix<Number> &matrix,
                         FullMatrix<Number>       &Q)
{
  AssertDimension(matrix.m(), matrix.n());
...
template <typename Number>
Vector<Number>
compute_eigenvalues_symm(const FullMatrix<Number> &matrix,
                         FullMatrix<Number>       &Q)
{
  LAPACKFullMatrix<Number> lapack_matrix(matrix.m());
  lapack_matrix = matrix;
                    ...
                    const Number threshold = zero_threshold<Number>)
{
  ...
  for (auto i = 0U; i < n_singular_values; ++i)
    invSigma(i, i) =
      inverse_scalar_if_impl(matrix.singular_value(i), threshold);
...
{
  fill_matrix_with_random_values<MatrixType, Number>(matrix,
                                                     matrix.m(),
                                                     matrix.n());
}
...
template <typename Number>
Vector<Number>
array_view_to_vector(const ArrayView<const Number> &view,
                     ...
                     const unsigned int                              lane = 0)
{
  ...
  std::transform(view.cbegin(),
                 view.cend(),
                 vec.begin(),
                 [lane](const auto &elem) { return elem[lane]; });
  return vec;
}
...
template <typename Number>
Vector<Number>
array_view_to_vector(const ArrayView<VectorizedArray<Number>> &view,
                     const unsigned int                        lane = 0)
{
...
to_string_conditional(const FloatType value)
{
  static_assert(std::is_floating_point<FloatType>::value,
                "FloatType is not floating point type.");
...
{
  AssertThrow(label.length() < size + 1,
              ExcMessage("String length exceeds column size!"));
...
struct Printer
{
  ...
  print_row_impl(OSTREAM &out, const unsigned, Arg &&arg)
  {
    ...
    out << fstring_column(std::forward<Arg>(arg),
                          ostream.str().length() + 4 + 3 + 1)
        << std::endl;
  }
  ...
  template <typename Arg, typename... Args>
  void
  print_row_variable_impl(OSTREAM       &out,
                          const unsigned size,
                          Arg          &&arg,
                          Args &&...args)
  {
...
{
  Printer<ConditionalOStream>{}.print_row_impl(out,
                                               size,
                                               std::forward<Arg>(arg));
}
...
{
  Printer<std::ostream>{}.print_row_impl(out,
                                         size,
                                         std::forward<Args>(args)...);
}
...
{
  Printer<ConditionalOStream>{}.print_row_variable_impl(out,
                                                        std::forward<Arg>(arg));
}
...
{
  Printer<ConditionalOStream>{}.print_row_variable_impl(
...
{
  Printer<std::ostream>{}.print_row_variable_impl(out,
                                                  size,
                                                  std::forward<Arg>(arg));
}
