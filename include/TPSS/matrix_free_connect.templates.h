namespace TPSS
{
    ...
    const std::map<unsigned int, unsigned int>        &dofh_index_map_in)
  {
    ...
    for (const auto &dof_info : dof_infos_in)
      {
        Assert(patch_info_0 == dof_info.patch_info,
               ExcMessage("PatchInfos do not coincide."));
        (void)dof_info;
      }
    ...
    const unsigned int n_cells_stored =
      proc_has_cells_on_level ? tria.n_cells(level) : 0;
     ...
     * vectorization. In the following, we map cells identified by its cell
     * index to their counterpart in the MatrixFree object identified by the
     * pair of batch index (bindex) and vectorization lane (bcomp).
     */
    std::vector<std::pair<unsigned int, unsigned int>>
      cindex_to_bindex_bcomp_pair;
    cindex_to_bindex_bcomp_pair.resize(
      n_cells_stored); // we don't care about the accurate size
    for (unsigned int bid = 0; bid < n_cell_batches; ++bid)
      for (unsigned int comp = 0; comp < macro_size; ++comp)
        if (comp < mf_storage->n_active_entries_per_cell_batch(bid))
          {
            const auto cell =
              mf_storage->get_cell_iterator(bid, comp, /*dofh_index*/ 0);
     ...
     * vectorization lane representing the same cell in the MatrixFree
     * framework.
     */
    ...
    for (auto i = 0U; i < n_cells_patch_info; ++i)
      {
        const auto [cell_index, dummy] =
          patch_info->get_cell_level_and_index(i);
        (void)dummy;
    ...
    for (auto dofh_index = 0U; dofh_index < dof_infos.size(); ++dofh_index)
      {
        ...
        auto       &cell_and_patch_dof_indices =
          dof_info_local.cell_and_patch_dof_indices;
        auto &dof_starts = dof_info_local.dof_starts;

        const auto dof_layout =
          TPSS::get_dof_layout(dof_info.dof_handler->get_fe());
        const unsigned int n_components =
          dof_info.dof_handler->get_fe().n_components();
        const bool is_scalar_Q_element =
          dof_layout == TPSS::DoFLayout::Q && n_components == 1U;
        if (is_scalar_Q_element)
          {
            TPSS::PatchDoFWorker<dim, Number> patch_dof_worker(dof_info);
            const auto                        fe_order =
              dof_info.dof_handler->get_fe().tensor_degree() + 1;
            PatchLocalHelperQ<dim> local_helper(fe_order);
            const auto             n_subdomains =
              patch_dof_worker.get_partition_data().n_subdomains();
            const auto n_physical_subdomains =
              patch_dof_worker.n_physical_subdomains();
            const auto regular_patch_size =
              patch_dof_worker.n_cells_per_subdomain();
            ...
            for (auto patch_id = 0U; patch_id < n_subdomains; ++patch_id)
              for (auto lane = 0U;
                   lane < patch_dof_worker.n_lanes_filled(patch_id);
                   ++lane)
                {
                  local_helper.reinit(
                    patch_dof_worker.fill_face_infos(patch_id, lane));

                  /// fill global_dof_index -> patch_dof_index map
                  std::map<types::global_dof_index, unsigned int>
                              global_to_patch_dof_index;
                  const auto &global_dof_indices =
                    patch_dof_worker.get_dof_indices_on_patch(patch_id, lane);
                  for (auto patch_dof_index = 0U;
                       patch_dof_index < global_dof_indices.size();
                       ++patch_dof_index)
                    {
                      const auto global_dof_index =
                        global_dof_indices[patch_dof_index];
                      const auto [it, is_inserted] =
                        global_to_patch_dof_index.emplace(global_dof_index,
                                                          patch_dof_index);
                      (void)it;
                      (void)is_inserted;
                      Assert(
                  ...
                  for (auto cell_no = 0U; cell_no < local_helper.n_cells();
                       ++cell_no)
                    {
                      /// Note this field returns all global dof indices on the
                      /// cell even those not being part of the patch.
                      AssertThrow(
                        dof_info.n_dofs_on_cell_per_comp.size() == 1,
                        ExcMessage(
                          "TODO not implemented for vector-valued shape functions.")); // !!!
                      const auto &global_dof_indices_on_cell =
                        patch_dof_worker.get_dof_indices_on_cell(
                          patch_id, cell_no, lane, /*!!! component*/ 0);
                      AssertDimension(global_dof_indices_on_cell.size(),
                                      local_helper.n_dofs_per_cell(cell_no));
                      const auto n_dofs_per_cell_on_patch =
                        local_helper.n_dofs_per_cell_on_patch(cell_no);
                      const auto &cell_dof_indices =
                        local_helper.get_cell_dof_indices(cell_no);

                      dof_starts.emplace_back(
                        cell_and_patch_dof_indices.size());
                      for (auto cell_dof_no = 0U;
                           cell_dof_no < n_dofs_per_cell_on_patch;
                           ++cell_dof_no)
                        {
                          const auto cell_dof_index =
                            cell_dof_indices[cell_dof_no];
                          const auto global_dof_index =
                            global_dof_indices_on_cell[cell_dof_index];
                          const auto patch_dof_index =
                            global_to_patch_dof_index[global_dof_index];
                          cell_and_patch_dof_indices.emplace_back(
                            cell_dof_index, patch_dof_index);
                        }
                    }
                }
