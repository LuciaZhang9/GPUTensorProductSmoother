...
void
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::evaluate(
      ...
      for (unsigned int cell_no = 0; cell_no < n_cells_per_direction; ++cell_no)
        {
          ...
          for (unsigned int dof = 0; dof < n_dofs_per_cell_1d; ++dof)
            {
              const auto *unit_grad_begin =
                unit_grads_begin + dof * n_q_points_1d;
              auto *grad = &(shape_gradient_impl(dof, 0, d, cell_no));
              std::transform(unit_grad_begin,
                             unit_grad_begin + n_q_points_1d,
                             grad,
                             [h_inv](const auto &unit_grad) {
                               return unit_grad * h_inv;
                             });
            }

          for (const int face_no : {0, 1})
            {
              const auto *unit_grads_on_face =
                shape_data.shape_data_on_face[face_no].begin() + fe_order;
              auto *grad_on_face =
                &(shape_gradient_face_impl(0, face_no, d, cell_no));
      ...
      for (unsigned int cell_no = 0; cell_no < n_cells_per_direction; ++cell_no)
        {
          ...
          for (unsigned int dof = 0; dof < n_dofs_per_cell_1d; ++dof)
            {
              const auto *unit_hessian_begin =
                unit_hessians_begin + dof * n_q_points_1d;
              auto *hessian = &(shape_hessian_impl(dof, 0, d, cell_no));
              std::transform(unit_hessian_begin,
                             unit_hessian_begin + n_q_points_1d,
                             hessian,
                             [h_inv](const auto &unit_hessian) {
                               return unit_hessian * h_inv * h_inv;
                             });
            }
          ...
          for (const int face_no : {0, 1})
            {
              ...
              auto *hessian_on_face =
                &(shape_hessian_face_impl(0, face_no, d, cell_no));
...
{
  auto matrices =
    patch_action_impl(eval_ansatz, std::forward<CellOperation>(cell_operation));
...
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::patch_action(
  ...
  const InterfaceOperation &interface_operation) const
{
  auto matrices = patch_action_impl(eval_ansatz,
                                    cell_operation,
                                    face_operation,
                                    interface_operation);
  ...
  for (int direction = 0; direction < dim; ++direction)
    {
      ...
      const auto n_dofs_plain_1d_ansatz =
        eval_ansatz.n_dofs_plain_1d(direction);
      const auto n_dofs_per_cell_1d_test = this->n_dofs_per_cell_1d(direction);
      const auto n_dofs_per_cell_1d_ansatz =
        eval_ansatz.n_dofs_per_cell_1d(direction);
      ...
      for (unsigned int cell_no = 0; cell_no < n_cells_1d(direction); ++cell_no)
        {
          MatrixType cell_matrix(n_dofs_per_cell_1d_test,
                                 n_dofs_per_cell_1d_ansatz);
          std::forward<decltype(cell_operation)>(cell_operation)(
            eval_ansatz, *this, cell_matrix, direction, cell_no);
  ...
  for (int direction = 0; direction < dim; ++direction)
    {
      ...
      for (unsigned int cell_no = 0; cell_no < n_cells_1d(direction); ++cell_no)
        {
          ...
          for (const int face_no : {0, 1})
            {
              MatrixType cell_matrix(n_dofs_per_cell_1d_test,
                                     n_dofs_per_cell_1d_ansatz);
              std::forward<decltype(face_operation)>(face_operation)(
                eval_ansatz, *this, cell_matrix, direction, cell_no, face_no);
              /// TODO note, it is safer to submit after each operation but not
              /// efficient ...
              submit_cell_matrix(matrix,
                                 cell_matrix,
                                 cell_no,
          ...
          const bool has_adjacent_cell_on_rhs =
            cell_no + 1 < n_cells_1d(direction);
          if (has_adjacent_cell_on_rhs)
            {
              const auto cell_no_left  = cell_no;
              const auto cell_no_right = cell_no + 1;
              MatrixType cell_matrix_leftright(n_dofs_per_cell_1d_test,
                                               n_dofs_per_cell_1d_ansatz);
              MatrixType cell_matrix_rightleft(n_dofs_per_cell_1d_test,
                                               n_dofs_per_cell_1d_ansatz);
              std::forward<decltype(interface_operation)>(
                interface_operation)(eval_ansatz,
                                     *this,
      ...
      AssertDimension(this->n_dofs_plain_1d(direction),
                      matrices.at(direction).n_rows());
      AssertDimension(eval_ansatz.n_dofs_plain_1d(direction),
    ...
    &eval_ansatz) const
{
  ...
  AssertThrow(false,
              ExcMessage("Post processing of constraints is not implemented."));
  return;
}
  ...
  const TPSS::PatchDoFWorker<dim, Number> &patch_worker_col) const
{
  ...
  const auto &additional_data_row =
    patch_worker_row.get_dof_info().get_additional_data();
  const auto &additional_data_col =
    patch_worker_col.get_dof_info().get_additional_data();
  const auto n_rows_restricted =
    additional_data_row.force_no_boundary_condition ?
      subdomain_matrix_plain.n_rows() :
      subdomain_matrix_plain.n_rows() - 2;
  const auto n_cols_restricted =
    additional_data_col.force_no_boundary_condition ?
  ...
  for (auto i = 0U; i < subdomain_matrix.n_rows(); ++i)
    for (auto j = 0U; j < subdomain_matrix.n_cols(); ++j)
      {
        const auto ii =
          additional_data_row.force_no_boundary_condition ? i : i + 1;
        const auto jj =
          additional_data_col.force_no_boundary_condition ? j : j + 1;
  ...
  const TPSS::PatchLocalTensorHelper<dim> *patch_dof_tensor_ansatz) const
{
  ...
  const auto n_dofs_per_cell_1d_row =
    patch_dof_tensor_row.n_dofs_per_cell_1d(dimension);
  const auto n_dofs_per_cell_1d_col =
    patch_dof_tensor_col.n_dofs_per_cell_1d(dimension);
  /// assuming isotropy ...
  ...
  AssertDimension(patch_dof_tensor_row.n_dofs_1d(dimension),
                  subdomain_matrix.n_rows());
  AssertDimension(patch_dof_tensor_col.n_dofs_1d(dimension),
                  subdomain_matrix.n_cols());
  for (unsigned int i = 0; i < n_dofs_per_cell_1d_row; ++i)
    for (unsigned int j = 0; j < n_dofs_per_cell_1d_col; ++j)
      {
        const auto ii =
          patch_dof_tensor_row.dof_index_1d(cell_no_row, i, dimension);
        const auto jj =
          patch_dof_tensor_col.dof_index_1d(cell_no_col, j, dimension);
