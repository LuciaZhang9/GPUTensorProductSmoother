namespace TPSS
{
    ...
    LinearAlgebra::distributed::Vector<Number> &vec) const
  {
    ...
    Assert(dof_info->vector_partitioner,
           ExcMessage("vector_partitioner is not set."));
  ...
  PatchDoFWorker<dim, number>::initialize_vector_partitioner() const
  {
    ...
    const auto caching_strategy =
      dof_info->get_additional_data().caching_strategy;
    ...
    const auto partitioner =
      std::make_shared<Utilities::MPI::Partitioner>(owned_indices,
                                                    MPI_COMM_WORLD);
