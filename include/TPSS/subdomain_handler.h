...
public:
  ...
  void
  reinit(const dealii::MatrixFree<dim, Number> *mf_storage,
...
template <int dim, typename Number>
unsigned int
SubdomainHandler<dim, Number>::get_unique_dofh_index(
  const unsigned int dofh_index) const
{
...
SubdomainHandler<dim, Number>::get_dof_handler(
  const unsigned int dofh_index) const
{
  const auto unique_dofh_index = get_unique_dofh_index(dofh_index);
  Assert(unique_dof_infos[unique_dofh_index].dof_handler,
         ExcMessage("DoFHandler is not set."));
...
SubdomainHandler<dim, Number>::get_dof_layout(
  const unsigned int dofh_index) const
{
...
SubdomainHandler<dim, Number>::get_shape_info(
  const unsigned int dofh_index) const
{
...
SubdomainHandler<dim, Number>::get_vector_partitioner(
  const unsigned int dofh_index) const
{
  AssertIndexRange(dofh_index, n_dof_handlers());
  const auto &dof_info = get_dof_info(dofh_index);
