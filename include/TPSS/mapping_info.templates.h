...
namespace TPSS
{
    ...
    const unsigned int                patch_id) const
  {
    ...
    typename MappingInfo<dim, number>::LocalData local_data(
  ...
  template <int dim, typename number>
  typename MappingInfo<dim, number>::LocalData
  MappingInfo<dim, number>::compute_average_scaling(
    FEValues<dim>                    &fe_values,
    const PatchMFWorker<dim, number> &patch_worker,
    const unsigned int                patch_id) const
  {
    ...
    const auto cell_collection{
      std::move(patch_worker.get_cell_collection(patch_id))};
    typename MappingInfo<dim, number>::LocalData local_data(
      n_cells_per_direction);
      ...
      for (unsigned int d = 0; d < dim; ++d)
        {
          ...
          if (additional_data
                .use_arc_length) // compute arc length between opposite faces
            for (unsigned int qf = 0; qf < n_qpoints_face; ++qf)
              {
                const auto qf_multi =
                  Tensors::uni_to_multiindex<dim - 1>(qf, qmode_size);
                const auto qfibre =
                  Tensors::index_fibre<dim>(qf_multi, /*mode*/ d, qmode_size);
                auto arc_length = make_vectorized_array<double>(0.);
                for (unsigned int i = 1; i < qfibre.size(); ++i)
                  {
                    const auto &delta =
                      qpoints[qfibre[i - 1]] - qpoints[qfibre[i]];
              ...
              for (unsigned int qf = 0; qf < n_qpoints_face; ++qf)
                {
                  ...
                  const auto qfibre =
                    Tensors::index_fibre<dim>(qf_multi, /*mode*/ d, qmode_size);
                  const auto &delta =
                    qpoints[qfibre.front()] - qpoints[qfibre.back()];
                  h += make_vectorized_array<double>(fweights[qf]) *
                       delta.norm_square();
                }
              h = std::sqrt(h);
            }
        ...
        for (unsigned int face_no = 0;
             ...
             ++face_no)
          {
            ...
            Assert(cell->face_orientation(face_no),
                   ExcMessage("Invalid face orientation."));
            Assert(!cell->face_flip(face_no), ExcMessage("Invalid face flip."));
            Assert(!cell->face_rotation(face_no),
                   ExcMessage("Invalid face rotation."));
          }
    };

    // *** compute the average scaling of each cell
    std::array<AlignedVector<VectorizedArray<number>>, dim> avg_cell_lengths;
    avg_cell_lengths.fill(
      AlignedVector<VectorizedArray<number>>(cell_collection.size()));
        ...
        if (additional_data.normalize_patch)
          {
            ...
            // // DEBUG
            // std::cout << "vol_actual = " << varray_to_string(volume_act) <<
            // std::endl; std::cout << "volume_surrogate = " <<
            // varray_to_string(volume_sur) << std::endl;
            const auto alpha =
              std::pow(volume_act / volume_sur,
                       1. / static_cast<double>(dim)); // normalization
            ...
            // // DEBUG
            // std::cout << "volume_newrogate = " <<
            // varray_to_string(volume_new) << std::endl; for (unsigned int d=0;
            // d<dim; ++d) 	std::cout << "h = " <<
            // varray_to_string(avg_cell_lengths[/*direction*/d][cell_no]) <<
            // std::endl;
          }
      }
    ...
    if (cell_collection.size() == 1) // CELL PATCHES
      for (unsigned int direction = 0; direction < dim; ++direction)
        local_data.h_lengths[direction][/*cell_no_1d*/ 0] =
          avg_cell_lengths[direction][0];
    ...
    else if (cell_collection.size() == 1 << dim) // VERTEX PATCHES
      {
        ...
        h.fill({make_vectorized_array<number>(0.),
                make_vectorized_array<number>(0.)});
        for (unsigned int cell_no = 0; cell_no < cell_collection.size();
             ++cell_no)
          for (unsigned int direction = 0; direction < dim; ++direction)
            {
              std::bitset<dim> binary_cell_no(cell_no);
              const auto       cell_no_1d =
                static_cast<std::size_t>(binary_cell_no[direction]);
              h[direction][cell_no_1d] +=
                avg_cell_lengths[direction][cell_no] / N;
            }
        for (unsigned int direction = 0; direction < dim; ++direction)
          for (unsigned int cell_no_1d = 0;
               cell_no_1d < /*n_cells_per_direction*/ 2;
               ++cell_no_1d)
            local_data.h_lengths[direction][cell_no_1d] =
              h[direction][cell_no_1d];
            ...
            for (unsigned int d = 0; d < dim; ++d)
              {
                ...
                volume_patch_sur *=
                  std::accumulate(lengths_1d.cbegin(),
                                  lengths_1d.cend(),
                                  make_vectorized_array<double>(0.));
              }
            // // DEBUG
            // std::cout << "vol_actual = " << varray_to_string(volume_patch) <<
            // std::endl; std::cout << "volume_surrogate = " <<
            // varray_to_string(volume_patch_sur) << std::endl; for (unsigned
            // int d=0; d<dim; ++d) 	print_row_variable (std::cout, 10,
            // "direction:", 5, d, 5, "h = ", 20, varray_to_string(h[d][0]), 20,
            // varray_to_string(h[d][1]));
            const auto alpha = std::pow(volume_patch / volume_patch_sur,
            ...
            for (unsigned int direction = 0; direction < dim; ++direction)
              {
                ...
                for (auto h = h_lengths.begin(); h != h_lengths.end();
                     ++h_old, ++h)
                  *h = ((*h_old) * alpha);
              }
          }
  ...
  template <int dim, typename number>
  void
  MappingInfo<dim, number>::initialize_storage(
    ...
    const AdditionalData                 &addit_data)
  {
    ...
    if (is_mpi_parallel && patch_info.empty())
      ...
      AssertThrow(!patch_info.empty(),
                  ExcMessage("No data stored in the PatchInfo."));
    ...
    if (patch_variant == PatchVariant::cell)
      ...
      patch_size = 1 << dim;
    n_cells_per_direction = (patch_variant == PatchVariant::cell)   ? 1 :
                            (patch_variant == PatchVariant::vertex) ? 2 :
                                                                      -1;
    ...
    this->mf_mapping_data =
      &internal::MatrixFreeFunctions::MappingInfoCellsOrFaces<
        dim,
        number,
        false /*is_face*/,
        VectorizedArrayType>::get(*mf_mapping_info, 0 /*quad_no*/);
    ...
    for (unsigned int color = 0; color < partition_data.n_colors(); ++color)
      for (unsigned int part = 0; part < partition_data.n_partitions(color);
           ++part)
        {
          const auto range = partition_data.get_patch_range(part, color);
          for (unsigned int patch_id = range.first; patch_id < range.second;
               ++patch_id)
            {
              PatchType   patch_type = PatchType::cartesian;
              const auto &batch_collection =
                patch_worker.get_batch_collection(patch_id);
              for (const auto &macro_pair : batch_collection)
                for (const auto &batch_pair : macro_pair)
                  {
                    const auto batch_id = batch_pair.first;
                    const auto cell_type =
                      mf_mapping_info->get_cell_type(batch_id);
                    // std::cout << "cell_type[" << patch_id << "]: " <<
                    // cell_type << std::endl;
                    patch_type =
                      patch_type < cell_type ? cell_type : patch_type;
                  }
              // std::cout << "patch_type[" << patch_id << "]: " << patch_type
              // << std::endl;
              ...
              if (patch_type == PatchType::cartesian)
                {
                  const auto &local_data =
                    extract_cartesian_scaling(fe_values,
                                              patch_worker,
                                              patch_id);
                  mapping_data_starts.emplace_back(
                    internal_data.h_lengths.size());
                  submit_local_data(local_data);
                }

              //  we have to compute average scalings to gain a tensor product
              //  structure
              else
                {
                  const auto &local_data =
                    compute_average_scaling(fe_values, patch_worker, patch_id);
                  mapping_data_starts.emplace_back(
                    internal_data.h_lengths.size());
                  submit_local_data(local_data);
                }
    ...
    AssertDimension(internal_data.h_lengths.size() %
                      (n_cells_per_direction * dim),
                    0);
