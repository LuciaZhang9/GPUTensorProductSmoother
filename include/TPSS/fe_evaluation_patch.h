...
namespace TPSS
{
  ...
  {
  public:
    using cell_evaluator_type =
      FEEvaluation<dim, fe_degree, n_q_points_1d, 1, Number>;
     ...
     */
    void
    gather(const unsigned int                              cell_no,
    ...
    /**
     * TODO...  How do we treat cell evaluator: inherit? cheap enough to
     * multiply over all patches?
     */
  ...
  FEEvaluationPatch<dim, fe_degree, n_q_points_1d, Number>::FEEvaluationPatch(
    ...
    : PatchMFWorker<dim, Number>(sd_handler_in.get_matrixfree_connect(),
                                 dofh_index)
    , cell_eval(
        std::make_shared<cell_evaluator_type>(sd_handler_in.get_matrix_free(),
                                              dofh_index))
  {}
  ...
  template <int dim, int fe_degree, int n_q_points_1d, typename Number>
  inline void
  FEEvaluationPatch<dim, fe_degree, n_q_points_1d, Number>::reinit(
    const unsigned int patch_id,
    const unsigned int patch_lane)
  {
    AssertIndexRange(patch_id,
                     patch_worker_type::get_partition_data().n_subdomains());
  ...
  FEEvaluationPatch<dim, fe_degree, n_q_points_1d, Number>::gather(
    ...
    const ArrayView<const VectorizedArray<Number>> &src)
  {
    AssertIndexRange(current_patch_id,
                     patch_worker_type::get_partition_data().n_subdomains());
    AssertIndexRange(current_patch_lane,
                     patch_worker_type::n_lanes_filled(current_patch_id));
    AssertDimension(src.size(),
                    patch_worker_type::n_dofs_on_patch(current_patch_id,
                                                       current_patch_lane));
    ...
    const auto &cell_and_patch_dof_indices =
      patch_worker_type::get_local_cell_and_patch_dof_indices(
        current_patch_id, cell_no, current_patch_lane);
    for (const auto [cell_dof_index, patch_dof_index] :
         cell_and_patch_dof_indices)
      cell_eval->submit_dof_value(src[patch_dof_index][current_patch_lane],
                                  cell_dof_index);
  }
    ...
    const ArrayView<VectorizedArray<Number>> &dst)
  {
    ...
    //                                                current_cell_no,
    //                                                current_patch_lane));
    AssertIndexRange(current_cell_no,
                     patch_worker_type::n_cells_per_subdomain());
    ...
    const auto &cell_and_patch_dof_indices =
      patch_worker_type::get_local_cell_and_patch_dof_indices(
        current_patch_id, current_cell_no, current_patch_lane);
    for (const auto [cell_dof_index, patch_dof_index] :
         cell_and_patch_dof_indices)
