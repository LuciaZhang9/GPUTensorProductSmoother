...
namespace TPSS
{
  ...
  public:
     ...
     */
    bool
    at_patch_boundary(const unsigned int cell_no,
                      const unsigned int cell_dof_index) const;

    /**
     * Returns the set of cell dof indices at cell @p cell_no being part of the
     * patch. This includes also the indices which are uniquely assigned to
     * other cells of the patch.
     */
  ...
  public:
    ...
    /**
     * Returns the one-dimensional patch local dof index with cell local dof
     * index
    ...
    dof_range_1d(const unsigned int cell_no,
  ...
  private:
     ...
     */
    // !!! TODO the current implementation is not correct. actually this
    // function
  ...
  /**
   * Helps with the d-dimensional patch local dof indexing depending on the
   * finite element.
   */
  template <int n_dimensions>
  ...
  public:
     ...
     */
    bool
    is_boundary_face_dof_1d(const unsigned int patch_dof_index,
                            const unsigned int dimension) const;

    /**
     * Returns the patch local dof index as multi-index subject to
     * lexicographical ordering. For more details see
     * PatchLocalIndexHelper::dof_index_1d.
     */
    std::array<unsigned int, n_dimensions>
    dof_multi_index(const unsigned int cell_no,
                    const unsigned int cell_dof_index) const;
     ...
     */
    unsigned int
    dof_index(const unsigned int cell_no,
              const unsigned int cell_dof_index) const;
   ...
   */
  // !!! TODO do not pass level as additional data, directly use the level of
  // patch_info_in
  ...
  struct DoFInfo
  {
    ...
    DoFAccessor<dim, dim, dim, true>
    get_level_dof_accessor_impl(const unsigned int cell_index,
     ...
     * of this array uniquely corresponds to a physical patch, which is
     * identified by its macro patch index and vectorization lane. The lane
     * index runs faster than the macro patch index.
     */
    std::vector<unsigned int> start_of_dof_indices_patchwise;

    /**
     * The array caches the proc-local global dof indices for each macro patch
     * stored in @p patch_info in flat format.
     */
  ...
  template <int n_dimensions>
  inline unsigned int
  PatchLocalIndexHelper<n_dimensions>::dof_index_1d(
  ...
  template <int n_dimensions>
  inline unsigned int
  PatchLocalIndexHelper<n_dimensions>::dof_index_1d_q_impl(
    const unsigned int cell_no,
    const unsigned int cell_dof_index,
    const int          dimension) const
  {
  ...
  template <int n_dimensions>
  inline unsigned int
  PatchLocalIndexHelper<n_dimensions>::dof_index_1d_dgq_impl(
    const unsigned int cell_no,
    const unsigned int cell_dof_index,
    const int          dimension) const
  {
  ...
  template <int n_dimensions>
  inline unsigned int
  PatchLocalIndexHelper<n_dimensions>::dof_index_1d_dgp_impl(
    const unsigned int cell_no,
    const unsigned int cell_dof_index,
    const int          dimension) const
  {
  ...
  PatchLocalIndexHelper<n_dimensions>::dof_range_1d(
    const unsigned int cell_no,
    const unsigned int dimension) const
  {
    ...
    const unsigned int last =
      dof_index_1d(cell_no, last_cell_dof_index, dimension);
    return {first, last + 1};
  }
  ...
  template <int n_dimensions>
  inline unsigned int
  PatchLocalIndexHelper<n_dimensions>::n_cells_1d(
    const unsigned int dimension) const
  {
  ...
  template <int n_dimensions>
  inline unsigned int
  PatchLocalIndexHelper<n_dimensions>::n_dofs_per_cell_1d(
    const unsigned int dimension) const
  {
  ...
  inline PatchLocalTensorHelper<n_dimensions>::PatchLocalTensorHelper(
    ...
    const Tensors::TensorHelper<n_dimensions> &cell_dof_tensor_in,
    const DoFLayout                            dof_layout_in)
    : IndexHelperBase(cell_tensor_in, cell_dof_tensor_in, dof_layout_in)
    , TensorHelperBase([&]() {
      ...
      for (auto d = 0U; d < n_dimensions; ++d)
        {
          ...
          sizes[d] =
            this->dof_index_1d(last_cell_no, last_cell_dof_index, d) + 1;
        }
      return sizes;
    }())
  {}
  ...
  template <int n_dimensions>
  inline bool
  PatchLocalTensorHelper<n_dimensions>::is_boundary_face_dof(
    const unsigned int patch_dof_index) const
  {
    const auto &patch_dof_index_multi =
      TensorHelperBase::multi_index(patch_dof_index);
    for (auto d = 0U; d < n_dimensions; ++d)
  ...
  PatchLocalTensorHelper<n_dimensions>::dof_multi_index(
    const unsigned int cell_no,
    const unsigned int cell_dof_index) const
  {
    const auto &cell_no_multi =
      IndexHelperBase::cell_tensor.multi_index(cell_no);
    const auto &cell_dof_index_multi =
      IndexHelperBase::cell_dof_tensor.multi_index(cell_dof_index);
    std::array<unsigned int, n_dimensions> patch_dof_index_multi;
    for (auto d = 0U; d < n_dimensions; ++d)
      patch_dof_index_multi[d] =
        IndexHelperBase::dof_index_1d(cell_no_multi[d],
                                      cell_dof_index_multi[d],
                                      d);
  ...
  template <int n_dimensions>
  inline unsigned int
  PatchLocalTensorHelper<n_dimensions>::dof_index(
    const unsigned int cell_no,
    const unsigned int cell_dof_index) const
  {
    const auto &patch_dof_index_multi =
      dof_multi_index(cell_no, cell_dof_index);
  ...
  template <int n_dimensions>
  inline bool
  PatchLocalTensorHelper<n_dimensions>::is_boundary_face_dof_1d(
  ...
  template <int n_dimensions>
  inline unsigned int
  PatchLocalTensorHelper<n_dimensions>::n_dofs_1d(
    const unsigned int dimension) const
  {
  ...
  {
    clear();
    std::copy(face_infos_in.cbegin(),
              face_infos_in.cend(),
              std::back_inserter(face_infos));
    ...
    for (auto cell_no = 0U; cell_no < n_cells(); ++cell_no)
      {
        ...
        for (auto cell_dof_index = 0U; cell_dof_index < n_cell_dofs;
             ++cell_dof_index)
  ...
  template <int n_dimensions>
  bool
  PatchLocalHelperQ<n_dimensions>::at_patch_boundary(
    const unsigned int cell_no,
    const unsigned int cell_dof_index) const
  {
    ...
    const auto &boundary_numbers =
      face_info.get_face_numbers_at_patch_boundary();
    for (const auto edge_no : edge_numbers)
      if (std::any_of(boundary_numbers.cbegin(),
                      boundary_numbers.cend(),
                      [edge_no](const auto boundary_no) {
                        return boundary_no == edge_no;
                      }))
        return true;
    return false;
  }
  ...
  template <int n_dimensions>
  unsigned int
  PatchLocalHelperQ<n_dimensions>::n_dofs_per_cell(
  ...
  template <int n_dimensions>
  unsigned int
  PatchLocalHelperQ<n_dimensions>::n_dofs_per_cell_on_patch(
    const unsigned int cell_no) const
  {
  ...
  template <int n_dimensions>
  ArrayView<const unsigned int>
  PatchLocalHelperQ<n_dimensions>::get_cell_dof_indices(
    const unsigned int cell_no) const
  {
    ...
    return make_array_view<const unsigned int>(cell_dof_indices_plain.data() +
                                                 start,
                                               cell_dof_indices_plain.data() +
                                                 end);
  }
  ...
  template <int n_dimensions>
  unsigned int
  PatchLocalHelperQ<n_dimensions>::get_dof_start_plain(
    const unsigned int cell_no) const
  {
  ...
  DoFInfo<dim, Number>::get_level_dof_accessor(
    const unsigned int cell_position) const
  {
    Assert(patch_info, ExcMessage("Patch info not initialized."));
    const auto [cell_level, cell_index] =
      patch_info->get_cell_level_and_index(cell_position);
  ...
  template <int dim, typename Number>
  inline DoFAccessor<dim, dim, dim, true>
  DoFInfo<dim, Number>::get_level_dof_accessor_impl(
    const unsigned int cell_index,
    const unsigned int level) const
  {
    const auto &tria = dof_handler->get_triangulation();
    return DoFAccessor<dim, dim, dim, true>(&tria,
                                            level,
                                            cell_index,
                                            dof_handler);
  }
  ...
  DoFInfo<dim, Number>::fill_level_dof_indices(
    const unsigned int cell_position) const
  {
