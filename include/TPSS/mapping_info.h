...
namespace TPSS
{
  ...
  private:
    ...
    const internal::MatrixFreeFunctions::
      MappingInfo<dim, number, VectorizedArrayType> *mf_mapping_info = nullptr;
    const internal::MatrixFreeFunctions::
      MappingInfoStorage<dim, dim, VectorizedArrayType> *mf_mapping_data =
        nullptr;
  ...
  struct MappingInfo<dim, number>::LocalData
  {
    ...
    // bool compare_scaling (const LocalData& other, const double
    // zero_tolerance)
    // {
    //   for (unsigned int direction = 0; direction < dim; ++direction)
    //     for (unsigned int cell = 0; cell < h_lengths[direction].size();
    //     ++cell)
    // 	for (unsigned int vv = 0; vv < VectorizedArray<number>::size(); ++vv)
    // 	  if (std::fabs (h_lengths[direction][cell][vv] -
    // other.h_lengths[direction][cell][vv]) > zero_tolerance) 	    return
    // false;
    //   return true;
    // }
    ...
    const double                                   zero_tolerance_double) const
  {
    ...
    for (unsigned int vv = 0; vv < macro_size; ++vv)
      for (unsigned int d = 1; d < dim; ++d)
        if (std::fabs(jac_0[0][0][vv] - jac_0[d][d][vv]) >
            zero_tolerance_double)
          {
            is_uniform = false;
            //	  std::cout << jac_0[0][0][vv] << "-" << jac_0[d][d][vv] <<
            // std::endl ;
            break;
          }
    return is_uniform;
    ...
    for (unsigned int vv = 0; vv < macro_size; ++vv)
      for (unsigned int d1 = 0; d1 < dim; ++d1)
        for (unsigned int d2 = 0; d2 < dim; ++d2)
          if (std::fabs(jac_0[d1][d2][vv] - jac[d1][d2][vv]) >
              zero_tolerance_double)
            {
              are_equal = false;
              //	  std::cout << jac_0[0][0][vv] << "-" << jac_0[d][d][vv] <<
  ...
  template <int dim, typename number>
  inline void
  MappingInfo<dim, number>::submit_local_data(
    const MappingInfo<dim, number>::LocalData &local_data)
  {
    const unsigned int n_cells_per_direction = local_data.h_lengths[0].size();
    for (unsigned int d = 0; d < dim; ++d)
      for (unsigned int cell_no_1d = 0; cell_no_1d < n_cells_per_direction;
           ++cell_no_1d)
