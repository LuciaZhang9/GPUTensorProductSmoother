...
namespace TPSS
{
  ...
  {
  public:
    static constexpr unsigned int macro_size =
    ...
    std::array<std::bitset<PatchWorker<dim, number>::macro_size>,
    ...
    get_cell_collection(const unsigned int patch_id,
                        const unsigned int lane) const;

    /**
     * Returns the collection of macro cells describing the (regular) macro
     * patch
  ...
  protected:
     ...
     * contained in PatchInfo, into interior/boundary, incomplete/complete
     * groups, of vectorized batches of physical subdomains, so-called macro
     * patches.
     */
    void
  ...
  {
    AssertThrow(patch_info_in.get_additional_data().patch_variant !=
                  TPSS::PatchVariant::invalid,
                ExcInvalidState());
    typename PatchInfo<dim>::PartitionData subdomain_partition_data;
    compute_partition_data(subdomain_partition_data,
                           patch_info_in.get_internal_data());
    const bool partition_data_is_valid = subdomain_partition_data.is_compatible(
      patch_info_in.subdomain_partition_data);
    (void)partition_data_is_valid;
    Assert(partition_data_is_valid,
           ExcMessage("The PartitionData does not fit the InternalData."));
  }
    ...
    , patch_variant(patch_info_in.get_additional_data().patch_variant)
  {
    ...
    compute_partition_data(subdomain_partition_data,
                           patch_info_in.get_internal_data());
    if (subdomain_partition_data.is_compatible(
          patch_info_in.subdomain_partition_data))
      return;
  ...
  PatchWorker<dim, number>::is_interior(const unsigned int patch) const
  {
    ...
    const bool is_interior_patch =
      std::all_of(bdry_masks.cbegin(), bdry_masks.cend(), [](const auto &mask) {
        return mask.none();
      });
  ...
  PatchWorker<dim, number>::n_physical_subdomains() const
  {
    ...
    const auto &subdomain_data =
      patch_info->get_internal_data()->subdomain_quantities_accumulated;
    const unsigned int n_subdomains =
      subdomain_data.n_interior + subdomain_data.n_boundary;
  ...
  template <int dim, typename number>
  inline unsigned int
  PatchWorker<dim, number>::n_lanes_filled_impl(
  ...
  PatchWorker<dim, number>::get_owned_range(
    const unsigned int color,
  ...
  template <int dim, typename number>
  inline std::pair<unsigned int, unsigned int>
  PatchWorker<dim, number>::get_ghost_range(
    const unsigned int color,
    const RangeVariant range_variant) const
  {
    const auto &partition_data   = get_partition_data();
  ...
  inline std::array<std::bitset<PatchWorker<dim, number>::macro_size>,
                    GeometryInfo<dim>::faces_per_cell>
  PatchWorker<dim, number>::get_at_boundary_masks(
    const unsigned int patch_id) const
  {
    Assert(patch_info != nullptr, ExcNotInitialized());
    AssertIndexRange(patch_id, get_partition_data().n_subdomains());
    ...
    // TODO patch face contains interior cell face and cell face at physical
    // boundary
    ...
    for (unsigned int d = 0; d < dim; ++d)
      {
        at_bdry_masks[d * 2] =
          get_mask(cell_collection.front(), d, /*face_no*/ 0);
        at_bdry_masks[d * 2 + 1] =
          get_mask(cell_collection.back(), d, /*face_no*/ 1);
      }
  ...
  template <int dim, typename number>
  inline std::array<
    std::array<types::boundary_id, PatchWorker<dim, number>::macro_size>,
    GeometryInfo<dim>::faces_per_cell>
  PatchWorker<dim, number>::get_boundary_ids(const unsigned int patch) const
  {
    ...
    const auto fill_bdry_ids = [&](const auto &macro_cell,
  ...
  template <int dim, typename number>
  inline std::vector<std::array<typename PatchWorker<dim, number>::CellIterator,
                                PatchWorker<dim, number>::macro_size>>
  PatchWorker<dim, number>::get_cell_collection(
    const unsigned int patch_id) const
  {
    ...
    const auto get_cell_position_filled = [&](const auto cell_no,
                                              const auto lane) {
      ...
      for (auto lane = 0U; lane < macro_size; ++lane)
        {
          ...
          cell_collect[cell_no][lane] =
            patch_info->get_cell_iterator(cell_position);
        }
    return cell_collect;
  }
    ...
    for (unsigned int color = 0; color < n_colors; ++color)
      {
        ...
        const auto subdomain_data =
          internal_data->subdomain_quantities.at(color);
        ...
        const unsigned int n_remaining_owned =
          (n_subdomains_owned % macro_size);
        {
          const unsigned int n_complete_batches =
            (n_subdomains_owned / macro_size);
        ...
        const unsigned int n_remaining_ghost =
          (n_subdomains_ghost % macro_size);
        {
          const unsigned int n_complete_batches =
            (n_subdomains_ghost / macro_size);
        ...
        if (patch_starts)
          {
            ...
            const unsigned int patch_size =
              UniversalInfo<dim>::n_cells(additional_data.patch_variant);

            // First, the data access with respect to locally owned subdomains
            // is set.
            {
              // complete range
              const unsigned int stride_comp = patch_size * macro_size;
              const auto         complete_range =
                partition_data.get_patch_range(0, color);
              for (unsigned int pp = complete_range.first;
                   pp < complete_range.second;
                   ++pp)
                {
              ...
              // incomplete range
              const auto incomplete_range =
                partition_data.get_patch_range(1, color);
              const unsigned int n_incomplete_batches =
                incomplete_range.second - incomplete_range.first;
            ...
            // Analogously, the data access with respect to subdomains with
            // ghost cells is set.
            {
              // complete range
              const unsigned int stride_comp = patch_size * macro_size;
              const auto         complete_range =
                partition_data.get_patch_range(2, color);
              for (unsigned int pp = complete_range.first;
                   pp < complete_range.second;
                   ++pp)
                {
              ...
              // incomplete range
              const auto incomplete_range =
                partition_data.get_patch_range(3, color);
              const unsigned int n_incomplete_batches =
                incomplete_range.second - incomplete_range.first;
              Assert(n_incomplete_batches == 0 || n_incomplete_batches == 1,
                     ExcMessage("Has more than one incomplete batch."));
    ...
    if (patch_starts)
      {
        ...
        patch_starts->emplace_back(
          start); // endpoint required by n_lanes_filled()
      }
  }
  ...
  PatchWorker<dim, number>::has_valid_state(
    const typename PatchInfo<dim>::PartitionData &partitions,
    const unsigned int                            color)
  {
