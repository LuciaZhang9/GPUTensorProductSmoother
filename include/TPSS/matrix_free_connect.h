...
namespace TPSS
{
  ...
  {
    /**
     * This struct stores the dof indices to transfer patch local vectors to
     * cell local vectors (gather or restrict) and vice versa (scatter or
     * prolongate).
     */
    struct DoFInfoLocal;
  ...
  MatrixFreeConnect<dim, Number>::get_dof_info(
  ...
  MatrixFreeConnect<dim, Number>::get_dof_info_local(
    const unsigned int dofh_index) const
  {
    AssertIndexRange(dofh_index, dofh_index_map.size());
    const auto unique_dofh_index = dofh_index_map.at(dofh_index);
