...
namespace TimeStat
{
  ...
  mean(const std::vector<TimeInfo> &timings)
  {
    ...
    const double time_sum =
      std::accumulate(timings.cbegin(),
                      timings.cend(),
                      0.,
                      [](const auto &sum, const auto &tinfo) {
   ...
   */
  void
  fprint_sample(ConditionalOStream                       &pcout,
                const std::vector<std::vector<TimeInfo>> &timings)
  {
    ...
    AssertThrow(std::all_of(timings.cbegin(),
                            timings.cend(),
                            [n_timings](const auto &sect) {
                              return sect.size() == n_timings;
                            }),
    ...
    for (auto sect = timings.cbegin(); sect != timings.cend(); ++sect, ++no)
      print_row_variable(
        pcout, 15, no, 50, sect->front().description, sect->front().unit);
    pcout << std::endl;
