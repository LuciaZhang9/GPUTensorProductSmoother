...
namespace Tensors
{
  ...
  class SchurComplementFast
    : public TensorProductMatrix<order, Number, n_rows_1d>
  {
  public:
                    ...
                    const int          lambda_rank)
    {
      std::vector<std::array<Table<2, Number>, order>> minus_C_Ainv_B;
      {
        ...
        scratch =
          Tensors::productT<order, Number>(ksvd_eigenvalues, eigenvectors);
        const auto &minus_eigenvectors =
          Tensors::scale<order, Number>(-1., eigenvectors);
        scratch = Tensors::product<order, Number>(minus_eigenvectors, scratch);
        scratch =
          Tensors::product<order, Number>(scratch,
                                          B_in.get_elementary_tensors());
        minus_C_Ainv_B =
          Tensors::product<order, Number>(C_in.get_elementary_tensors(),
                                          scratch);
      }
    ...
    std::vector<std::array<Table<2, Number>, order>>
    init_Z(
      const std::vector<std::array<Table<2, Number>, order>> &minus_C_Ainv_B,
      ...
      const int                                               lambda_rank)
    {
      ...
      const auto  sqrt_of_Lambda_ =
        std::make_shared<AlignedVector<Number>>(eigenvalues.size());
      ...
      Q = std::make_shared<TensorProductMatrix<order, Number, n_rows_1d>>(
        eigenvectors);
    ...
    {
      // const auto print_eigenvalues = [](const auto & tensors, const
      // std::string name) {
      //   TensorProductMatrix<order, Number, n_rows_1d> tpmat(tensors);
      //   const auto &                                  mat =
      //   table_to_fullmatrix(tpmat.as_table(), 0); const auto & eigenvalues =
      //   compute_eigenvalues(mat); std::cout << "Eigenvalues of " << name <<
      //   std::endl; std::cout << vector_to_string(eigenvalues) << std::endl;
      //   return eigenvalues;
      // };
      // const auto print_inverse_eigenvalues = [](const auto & tensors, const
      // std::string name) {
      //   TensorProductMatrix<order, Number, n_rows_1d> tpmat(tensors);
      //   const auto &                                  mat =
      //   table_to_fullmatrix(tpmat.as_table(), 0); const auto & eigenvalues =
      //   compute_eigenvalues(mat); std::vector<std::complex<double>>
      //   inverse_eigenvalues; std::transform(eigenvalues.begin(),
      //                  eigenvalues.end(),
      //                  std::back_inserter(inverse_eigenvalues),
      //                  [](const std::complex<double> & lambda) { return 1. /
      //                  lambda; });
      //   std::sort(inverse_eigenvalues.begin(),
      //             inverse_eigenvalues.end(),
      //             [](const auto & a, const auto & b) { return a.real() <
      //             b.real(); });
      ...
      // };
      // const auto print_eigenvalues_symm = [&](const Vector<double> &
      // eigenvalues,
      ...
      // };
      // const auto remove_zeros = [](std::vector<std::array<Table<2, Number>,
      // order>> & tensors) {
      //   std::vector<std::array<Table<2, Number>, order>> tmp;
      //   std::remove_copy_if(tensors.cbegin(),
      //                       tensors.cend(),
      ...
      AssertThrow(lambda_rank == -1 || lambda_rank > 0,
                  ExcMessage("Invalid lambda_rank."));
      AssertThrow(kronecker_rank == -1 || kronecker_rank > 0,
                  ExcMessage("Invalid kronecker_rank."));
      ...
      const auto lambda_rank_A =
        lambda_rank == -1 ? get_lambda_rank_max(A_in) : lambda_rank;
      ...
      if (kronecker_rank == -1)
        {
          ...
          std::copy(D_tensors.cbegin(),
                    D_tensors.cend(),
                    std::back_inserter(minus_C_Ainv_B));
      ...
      if (factor < 0.)
        {
          ...
          const int kronecker_rank_max =
            D_in.get_elementary_tensors().size() + minus_C_Ainv_B.size();
          const auto kronecker_rank_Sinv =
            kronecker_rank == -1 ? kronecker_rank_max : kronecker_rank;
          std::vector<std::array<Table<2, Number>, order>> inverse_ksvd(
            kronecker_rank_Sinv);
       ...
       *
       * Compute rank2 KSVD of Z in two steps: First we compute the rank1 KSVD
       * Z1 of Z. Afterwards we might scale the identity tensor of the remainder
      ...
      /// Compute rank1 KSVD of Z
      {
        ...
        const auto                                       lambda_rank_D =
          lambda_rank == -1 ? get_lambda_rank_max(D_in) : lambda_rank;
        ...
        /// positive definite
        else
          {
            ...
            while (
              less_than_all_lanes(min_lambda_estimate, static_cast<Number>(0.)))
              {
                // !!!
                pd_factor += delta;
                Z_tensors.front().front() =
                  LinAlg::scaling(get_identity_tensor(D_in).front(), pd_factor);
                compute_ksvd<Number>(Z_tensors, rank1_ksvd);
                // {
                ...
                const auto flag =
                  less_than(min_lambda_estimate, static_cast<Number>(0.));
                for (auto lane = 0U; lane < get_macro_size<Number>(); ++lane)
                  if (flag[lane])
                    {
                      const auto deviation =
                        abs(scalar_value(min_lambda_estimate, lane));
                      const auto prev_deviation =
                        scalar_value(previous_deviation, lane);
                      scalar_value(delta, lane) =
                        factor * deviation + 0.1 * prev_deviation;
        ...
        //   const auto matrix      =
        //   table_to_fullmatrix(Tensors::matrix_to_table(*this), 0); const auto
        //   eigenvalues = compute_eigenvalues(matrix); std::cout <<
        //   "Eigenvalues of (scaled = " << factor << ") rank2 KSVD of S" <<
        //   std::endl; std::cout << vector_to_string(eigenvalues) << std::endl;
        // }
      }
    }
          ...
          const ArrayView<const Number> &src_view) const
    {
      if (mode == Mode::exact)
      ...
      else if (mode == Mode::ksvd)
        {
          ...
          Assert(sqrt_of_Lambda,
                 ExcMessage("sqrt_of_Lambda isn't initialized."));
          Assert(!(sqrt_of_Lambda->empty()),
                 ExcMessage("sqrt_of_Lambda is empty"));
  ...
  public:
          ...
          const ArrayView<const Number> &src_view) const
    {
      ...
      const auto dst_view_of_AinvB =
        ArrayView(tmp_array.begin() + n_rows_max, A->n());
   ...
   *    | C D |
   *
   * using the Schur complement S for D, namely S = D - C * A^{-1} * B. The
   * inverse of the 2x2 block matrix is given by
   *
   *    | I   -A^{-1} * B | | A^{-1}   0      | | I             0 |
   *    | 0   I           | | 0        S^{-1} | | -C * A^{-1}   I |
   *
  ...
  public:
    ...
    BlockGaussianInverse(const matrix_type &A_in,
      ...
      , D(D_in)
    {
      // AssertThrow(!(kronecker_rank != 0 && lambda_rank == 0),
      // ExcMessage("lambda_rank is invalid."));
          ...
          const ArrayView<const Number> &src_view) const
    {
      ...
      /// src1[m0,m0+m1) <- [-CA^{-1}  I] src_view = - CA^{-1} src_view[0,m0) +
      /// src_view[m0,m0+m1)
  ...
  public:
    ...
    m() const
    {
      Assert(check_row_sizes(),
             ExcMessage("Inconsistent number of rows of block matrices."));
      std::size_t n_rows = 0;
                  ...
                  const ArrayView<const Number> &src) const
    {
      ...
      if (mode == 0) // fast mode
          ...
          if (!fast_inverse_2x2)
            {
              fast_inverse_2x2 = std::make_shared<
                BlockGaussianInverse<matrix_type, FastDiagSchurType>>(
                get_block(0, 0),
                get_block(0, 1),
                get_block(1, 0),
                get_block(1, 1),
        ...
        {
          if (!inverse_2x2)
            inverse_2x2 = std::make_shared<BlockGaussianInverse<matrix_type>>(
              get_block(0, 0),
              get_block(0, 1),
              get_block(1, 0),
              get_block(1, 1));
          ...
          // if(!basic_inverse)
          //   basic_inverse = std::make_shared<const
          //   InverseTable<Number>>(as_table());
  ...
  private:
                     ...
                     const ArrayView<const Number> &src) const
    {
      ...
      Assert(n_rows_1d == -1 || src.size() % n_rows_1d == 0,
             ExcMessage(
               "Input vector sizes are not a multiple of the static size."));
      ...
      for (std::size_t row = 0; row < n_block_rows(); ++row)
        {
          ...
          for (std::size_t col = 0; col < n_block_cols(); ++col)
            {
              const ArrayView<const Number> src_block(src.begin() + col_start,
                                                      n(col));
     ...
     */
    mutable std::shared_ptr<const BlockGaussianInverse<matrix_type>>
      inverse_2x2;
    /// ALTERNATIVE: standard inverse based on LAPACK
    ...
    mutable std::shared_ptr<
      const BlockGaussianInverse<matrix_type, FastDiagSchurType>>
           fast_inverse_2x2;
    ...
    m() const
    {
      const unsigned int n_rows =
        std::accumulate(blocks.begin(),
                        blocks.end(),
                        0,
                        [](const auto sum, const auto &matrix) {
                          return sum + matrix.m();
                        });
      return n_rows;
    }
  ...
  private:
                     ...
                     const ArrayView<const Number> &src) const
    {
      ...
      AssertThrow(
        n_rows_1d == -1 || src.size() % n_rows_1d == 0,
        ExcMessage(
          "Input vector sizes are not a multiple of the static size."));
      ...
      for (const auto &matrix : blocks)
        {
          ...
          row_end += matrix.m();
          col_end += matrix.n();
          const ArrayView<Number>       dst_block(dst.begin() + row_start,
                                            matrix.m());
          const ArrayView<const Number> src_block(src.begin() + col_start,
                                                  matrix.n());
  ...
  public:
    struct AdditionalData
    {
      ...
      /**
       * The number of rows/columns (pair.first/pair.second) of matrix A_d,
       * which is the Kronecker factor of dimension d (array position) for the
       * Kronecker product A_1 (x) A_0
       */
      std::array<std::pair<unsigned int, unsigned int>, 2>
                  size_of_kronecker_factors;
  ...
  class BlockGaussianInverseBasic
    : public BlockGaussianInverseBase<MatrixType,
                                      SchurComplementBasic<MatrixType, Number>,
                                      Number>
  {
  public:
    using Base =
      BlockGaussianInverseBase<MatrixType,
                               SchurComplementBasic<MatrixType, Number>,
                               Number>;
  ...
  public:
     ...
     * fresh inversion of the 0-0 block with default parameters when calling
     * this method.
     */
    void
    invert(const AdditionalData &additional_data          = AdditionalData{},
  ...
  private:
    std::shared_ptr<BlockGaussianInverseBasic<MatrixType, Number>>
      basic_inverse;
  };
  ...
  public:
    ...
    void
    invert(const typename InverseTable<Number>::AdditionalData &
             additional_data = typename InverseTable<Number>::AdditionalData{});
