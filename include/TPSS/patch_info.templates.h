namespace TPSS
{
                             ...
                             const AdditionalData   additional_data_in)
  {
    ...
    AssertIndexRange(additional_data_in.level,
    ...
    std::transform(internal_data.cell_iterators.cbegin(),
                   ...
                   [](const auto &cell) {
                     return std::make_pair<int, int>(cell->level(),
                                                     cell->index());
                   });
    internal_data.cell_iterators.clear();
    iterator_is_cached = !internal_data.cell_iterators.empty();
    level_and_index_is_cached =
      !internal_data.cell_level_and_index_pairs.empty();

    /// check validity
    AssertDimension(n_cells_stored_after_init, n_cells_plain());
    const auto n_colors_mpimin =
      Utilities::MPI::min(internal_data.n_colors(), MPI_COMM_WORLD);
    const auto n_colors_mpimax =
      Utilities::MPI::max(internal_data.n_colors(), MPI_COMM_WORLD);
    ...
    Assert(!internal_data.empty_on_all(),
           ExcMessage("No mpi-proc owns a patch!"));
  }


  template <int dim>
  void
  PatchInfo<dim>::initialize_cell_patches(
    ...
    const AdditionalData           additional_data)
  {
    ...
    /**
     * Coloring of the "cell patches". For the additive operator, we only have
     * one color. However, we require a vector of PatchIterators to call
     * submit_patches.
     */
    std::vector<std::vector<PatchIterator>> colored_iterators;
    constexpr int                           regular_size =
      UniversalInfo<dim>::n_cells(PatchVariant::cell);
    if (color_scheme == TPSS::SmootherVariant::additive) // ADDITIVE
      {
        ...
        for (auto patch = cell_collections.cbegin();
             patch != cell_collections.cend();
             ++patch)
     ...
     */
             ...
             TPSS::SmootherVariant::multiplicative) // MULTIPLICATIVE
      {
        ...
        if (do_graph_coloring) // graph coloring
          {
            ...
            colored_iterators = std::move(
              GraphColoring::make_graph_coloring(cell_collections.cbegin(),
          ...
          {
            colored_iterators = std::move(
              additional_data.coloring_func(cell_collections, additional_data));
          }
      }
    ...
    if (additional_data.visualize_coloring)
      additional_data.visualize_coloring(*dof_handler,
                                         colored_iterators,
                                         "cp_");
    ...
    if (additional_data.print_details)
      {
        print_row_variable(
          pcout, 45, "Coloring on level:", additional_data.level);
        ...
        for (unsigned c = 0; c < n_colors; ++c, ++subdomain_data)
          print_row_variable(pcout,
                             ...
                             c,
                             30,
                             subdomain_data->n_interior,
                             30,
                             subdomain_data->n_boundary);
        pcout << std::endl;
      }
  }
  ...
  PatchInfo<dim>::gather_vertex_patches(
  ...
  template <int dim>
  void
  PatchInfo<dim>::initialize_vertex_patches(
    const dealii::DoFHandler<dim> *dof_handler,
    const AdditionalData           additional_data)
  {
    constexpr auto regular_vpatch_size =
      UniversalInfo<dim>::n_cells(PatchVariant::vertex);
    const auto color_scheme = additional_data.smoother_variant;
    ...
    if (!additional_data.patch_distribution_func)
      cell_collections =
        std::move(gather_vertex_patches(*dof_handler, additional_data));
    else
      additional_data.patch_distribution_func(dof_handler,
                                              additional_data,
                                              cell_collections);
     ...
     * color as long as we do not use thread parallelism. In multi-threaded
     * loops race-conditions might occur due to overlap. Overlap means that two
     * adjacent FE subspaces share a common global degree of freedom.
     * Consequently, two local solvers might simultaneously write to the same
     * DoF entry in the destination vector. At the moment the user has to
     * provide a coloring scheme that prevents race conditions!
     */
            ...
            if (!additional_data.use_tbb)
              {
                ...
                for (auto it = cell_collections.cbegin();
                     it != cell_collections.cend();
                     ++it)
            ...
            else
              {
                ...
                colored_iterators =
                  std::move(additional_data.coloring_func(cell_collections,
                                                          additional_data));
              }
    ...
    // *** check if the InternalData is valid
    AssertDimension(internal_data.cell_iterators.size() % regular_vpatch_size,
                    0);
    // if(color_scheme == TPSS::SmootherVariant::additive)
    ...
    const unsigned int n_physical_subdomains =
      ...
      internal_data.subdomain_quantities_accumulated.n_boundary;
    (void)n_physical_subdomains;
    AssertDimension(n_physical_subdomains,
                    internal_data.cell_iterators.size() / regular_vpatch_size);

    if (additional_data.print_details &&
        color_scheme != TPSS::SmootherVariant::additive)
      {
      ...
      for (unsigned int face_no = 0;
           ...
           ++face_no)
        {
          const bool neighbor_has_same_level =
            (cell->neighbor_level(face_no) == cell->level());
          const bool neighbor_doesnt_exist =
            (cell->neighbor_level(face_no) == -1);
          const bool non_adaptive =
            neighbor_has_same_level || neighbor_doesnt_exist;
          (void)non_adaptive;
  ...
  void
  PatchInfo<dim>::submit_patches(
    const std::vector<PatchIterator> &patch_iterators)
  {
    // First, submit all interior patches and subsequently all boundary patches
    const auto &internal_submit =
      [this](const std::vector<PatchIterator> &patch_iterators) {
    ...
    for (const auto &patch : patch_iterators)
      {
        const bool patch_is_ghost =
          std::any_of(patch->cbegin(), patch->cend(), is_ghost_on_level);
        if (patch_is_ghost)
    ...
    subdomain_data.n_interior =
      owned_subdomain_data.n_interior + ghost_subdomain_data.n_interior;
    subdomain_data.n_boundary =
      owned_subdomain_data.n_boundary + ghost_subdomain_data.n_boundary;
  ...
  //   reordered_colors) const
  // {
  //   constexpr auto     regular_vpatch_size =
  //   UniversalInfo<dim>::n_cells(PatchVariant::vertex); const auto & tria =
  //   dof_handler.get_triangulation(); const unsigned int level               =
  //   internal_data.level;

  //   if(level == tria.n_levels() - 1)
  //   {
  //     GridOutFlags::Svg gridout_flags;
  //     gridout_flags.coloring           =
  //     GridOutFlags::Svg::Coloring::material_id;
  ...
  //     CellIterator       cell = dof_handler.begin_mg(level), end_cell =
  //     dof_handler.end_mg(level); for(; cell != end_cell; ++cell)
  ...
  //       filename             = filename + "L" +
  //       Utilities::int_to_string(level) + "_COLOR" +
  //                  Utilities::int_to_string(color, 2);
  //       std::ofstream output((filename + ".svg").c_str());

  //       unsigned int n_colored_cells = reordered_colors[color].first *
  //       regular_vpatch_size; for(unsigned int c = 0; c < n_colored_cells;
  //       ++cell_it, ++c)
  ...
  //         cell->set_material_id(void_id);
  //     }
  //     Assert(internal_data.cell_iterators.cend() == cell_it,
  //     ExcInternalError());
  //   }
  // }
