...
namespace TPSS
{
  ...
  public:
    ...
    std::set<unsigned int>
    get_constrained_local_dof_indices(const unsigned int patch_id,
                                      const unsigned int lane) const;
    ...
    /**
     * Same as above, but returning only global dof indices for vectorization
     * lane
    ...
    /**
     * Returns cached global dof indices on patch @patch_id at vectorization
     * lane
    ...
    fill_dof_indices_on_patch(const unsigned int patch_id,
                              const unsigned int lane) const;
     ...
     * global values. For restricted additive Schwarz methods (RAS) we might use
     * a partition of unity at the algebraic level: let $\tilde{R}_j^T$ denote
     * the restricted prolongation operator associated to the standard
     * prolongation $R_j^T$, then it holds $$\sum_j \R_j^T = id$$, where $id$ is
     * the identity.
     */
    std::vector<bool>
    get_restricted_dof_flags(const unsigned int patch_id,
                             const unsigned int lane) const;

    /**
     * TODO ... this method is not tested
     * the DoFInfo takes care of setting up a MPI::Partitioner during
     * initialization
     */
    ...
    /**
     * The MPI::Partitioner in the underlying DoFInfo is used to initialize
     * global dof vectors with respect to locally owned and ghosted dof indices.
     */
    void
    ...
    /**
     * Same as above except that in the serial case only the number of global
     * dofs
    ...
    /**
     * Number of degrees of freedom on patch (assuming same number on all
     * patches)
     * TODO...
     */
    unsigned int
  ...
  template <int dim, typename Number>
  inline PatchDoFWorker<dim, Number>::PatchDoFWorker(
    ...
    , n_components(dof_info_in.shape_info->n_components)
  {
    ...
    Assert(dof_info->get_additional_data().level !=
  ...
  PatchDoFWorker<dim, Number>::fill_dof_indices_on_patch(
    ...
    const unsigned int lane) const
  {
    ...
    const auto  n_cells =
      this
        ->n_cells_per_subdomain(); // !!! patch_dof_tensor.cell_tensor.n_flat();
    const auto n_dofs_per_cell_per_comp =
    ...
    if (is_H1_conforming)
        ...
        if (this->patch_variant == TPSS::PatchVariant::vertex)
          {
            ...
            /// ordering. Dof indices at the patch boundary are marked as
            /// invalid.
            const unsigned n_patch_dofs_per_component =
              patch_dof_tensor.n_flat();
            std::vector<unsigned int> global_dof_indices_plain(
              n_components * n_patch_dofs_per_component);
            for (auto comp = 0U; comp < n_components; ++comp)
              {
                AssertDimension(n_dofs_per_cell_per_comp, n_dofs_on_cell(comp));
                const unsigned int patch_dof_stride =
                  comp * n_patch_dofs_per_component;
                for (auto cell_no = 0U; cell_no < n_cells; ++cell_no)
                  {
                    ...
                    for (auto cell_dof_index = 0U;
                         cell_dof_index < n_dofs_per_cell_per_comp;
                         ++cell_dof_index)
                      {
                        const unsigned int patch_dof_index_per_comp =
                          patch_dof_tensor.dof_index(cell_no, cell_dof_index);
                        const bool is_boundary_dof =
                          patch_dof_tensor.is_boundary_face_dof(
                            patch_dof_index_per_comp);
                        global_dof_indices_plain[patch_dof_stride +
                                                 patch_dof_index_per_comp] =
                          is_boundary_dof ?
                            numbers::invalid_unsigned_int :
            ...
            /// Copy global dof indices neglecting all indices at the patch
            /// boundary (which have been marked in a previous step).
            std::copy_if(global_dof_indices_plain.cbegin(),
                         ...
                         [](const auto dof_index) {
                           return dof_index != numbers::invalid_unsigned_int;
                         });
          }
        else
    ...
    else if (is_L2_conforming)
      {
        const unsigned int n_patch_dofs_per_component =
          dof_layout == DoFLayout::DGP ?
            n_cells * get_shape_info().dofs_per_component_on_cell :
            patch_dof_tensor.n_flat();
        AssertIndexRange(n_patch_dofs_per_component,
                         patch_dof_tensor.n_flat() + 1);
        std::vector<unsigned int> global_dof_indices_plain(
          n_components * n_patch_dofs_per_component);
        for (auto comp = 0U; comp < n_components; ++comp)
          {
            ...
            for (auto cell_no = 0U; cell_no < n_cells; ++cell_no)
              {
                const auto global_dof_indices_on_cell =
                  get_dof_indices_on_cell(patch_id, cell_no, lane, comp);
                AssertDimension(n_dofs_per_cell_per_comp,
                                global_dof_indices_on_cell.size()); // ???
                for (auto cell_dof_index = 0U;
                     cell_dof_index < n_dofs_per_cell_per_comp;
                     ++cell_dof_index)
                  {
                    /// We dont obtain a patch-local lexicographical ordering
                    /// for DGP (TODO is it possible to have a truncated
    ...
    // if(dof_layout != DoFLayout::DGP) // !!! TODO n_dofs() is based on a full
    // tensor structure
  ...
  PatchDoFWorker<dim, Number>::get_constrained_local_dof_indices_1d(
    const unsigned int patch_id,
    const unsigned int dimension,
    const unsigned int lane) const
  {
    ...
    if (TPSS::PatchVariant::vertex == this->patch_variant)
    ...
    else if (TPSS::PatchVariant::cell == this->patch_variant)
      {
        ...
        const auto &additional_data = dof_info->get_additional_data();
        const auto  local_face_no   = [](const auto dimension,
                                      const auto face_no_1d) {
...

        {
          const types::boundary_id boundary_id =
            boundary_ids[local_face_no(dimension, 0)][lane];
          const bool at_dirichlet_boundary =
            additional_data.dirichlet_ids.find(boundary_id) !=
            additional_data.dirichlet_ids.cend();
          if (at_dirichlet_boundary)
            constrained_dof_indices.insert(first_dof_index);
        }
        {
          const types::boundary_id boundary_id =
            boundary_ids[local_face_no(dimension, 1)][lane];
          const bool at_dirichlet_boundary =
            additional_data.dirichlet_ids.find(boundary_id) !=
            additional_data.dirichlet_ids.cend();
          if (at_dirichlet_boundary)
  ...
  template <int dim, typename Number>
  inline std::set<unsigned>
  PatchDoFWorker<dim, Number>::get_constrained_local_dof_indices(
    const unsigned int patch_id,
    const unsigned int lane) const
  {
    ...
    for (auto dimension = 0U; dimension < dim; ++dimension)
      {
        ...
        for (const auto index : constrained_dof_indices_1d)
          {
            const auto &indices =
              patch_dof_tensor.sliced_indices(index, dimension);
  ...
  PatchDoFWorker<dim, Number>::get_dof_indices_on_cell(
    const unsigned int patch_id,
    const unsigned int cell_no,
    const unsigned int lane,
    const unsigned int component) const
  {
    ...
    const auto [dof_start, n_dofs] =
      get_dof_start_and_quantity_on_cell(patch_id, cell_no, lane);
    (void)n_dofs;
    AssertDimension(n_preceding_dofs_on_cell(n_components - 1) +
                      n_dofs_on_cell(n_components - 1),
                    n_dofs);
    ...
    const auto         begin =
      dof_info->dof_indices_cellwise.data() + dof_start + n_dofs_preceding;
  ...
  template <int dim, typename Number>
  inline std::array<ArrayView<const unsigned int>,
  ...
  template <int dim, typename Number>
  ArrayView<const unsigned int>
  PatchDoFWorker<dim, Number>::get_dof_indices_on_patch(
    const unsigned int patch_id,
    const unsigned int lane) const
  {
    AssertIndexRange(lane, this->n_lanes_filled(patch_id));
    Assert(dof_info, ExcMessage("dof_info is not set."));
    Assert(!(dof_info->dof_indices_patchwise.empty()),
           ExcMessage("Dof indices aren't cached."));

    const auto [dof_start, n_dofs] =
      get_dof_start_and_quantity_on_patch(patch_id, lane);
    ...
    const unsigned int component) const
  {
    ...
    const auto [dof_start, n_dofs] =
      get_dof_start_and_quantity_on_patch(patch_id, lane, component);
    const auto begin = dof_info->dof_indices_patchwise.data() + dof_start;
    return ArrayView<const unsigned int>(begin, n_dofs);
  }
  ...
  template <int dim, typename Number>
  std::vector<bool>
  PatchDoFWorker<dim, Number>::get_restricted_dof_flags(
    const unsigned int patch_id,
    const unsigned int lane) const
  {
    Assert(dof_info, ExcMessage("dof_info is not set."));
    ...
    const auto        begin =
      dof_info->restricted_dof_flags_patchwise.begin() + dof_start;
  ...
  PatchDoFWorker<dim, Number>::get_dof_start_and_quantity_on_patch(
    const unsigned int patch_id,
    const unsigned int lane) const
  {
    AssertIndexRange(lane, this->n_lanes_filled(patch_id));
    ...
    Assert(!(dof_info->dof_indices_patchwise.empty()),
           ExcMessage("No indices are cached."));
    AssertDimension(dof_info->start_of_dof_indices_patchwise.size(),
                    this->get_partition_data().n_subdomains() + 1);
    AssertIndexRange(patch_id, this->get_partition_data().n_subdomains());
    const unsigned int start =
      dof_info->start_of_dof_indices_patchwise[patch_id];
    const unsigned int end =
      dof_info->start_of_dof_indices_patchwise[patch_id + 1];
    ...
    const unsigned int component) const
  {
    ...
    const auto dof_start_at_component =
      dof_start + component * n_dofs_per_component;
  ...
  template <int dim, typename Number>
  inline std::pair<unsigned int, unsigned int>
  PatchDoFWorker<dim, Number>::get_dof_start_and_quantity_on_cell(
    const unsigned int patch_id,
    const unsigned int cell_no,
    const unsigned int lane) const
  {
    ...
    AssertIndexRange(cell_position,
                     dof_info->start_and_number_of_dof_indices_cellwise.size());
  ...
  PatchDoFWorker<dim, Number>::n_global_dofs() const
  {
    ...
    AssertDimension(vector_partitioner->size(),
                    dof_info->dof_handler->n_dofs(
                      dof_info->get_additional_data().level));
