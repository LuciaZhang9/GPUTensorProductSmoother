...
class FDEvaluation
{
  template <int dim_friend,
            int fe_degree_friend,
            int n_q_points_1d_friend,
            typename Number_friend>
  friend class FDEvaluation;

public:
  using CellAssembler = typename TPSS::
    MatrixEvaluator<fe_degree + 1, n_q_points_1d_, VectorizedArray<Number>>;
  ...
  get_average_factor(const int direction,
                     const int cell_no,
                     const int face_no) const;

  std::bitset<macro_size>
  get_boundary_mask(const int direction,
                    const int cell_no,
                    const int face_no) const;
  ...
  const internal::MatrixFreeFunctions::UnivariateShapeData<
    VectorizedArray<Number>> &
  get_shape_data(const unsigned int dimension,
  ...
  const VectorizedArray<Number> &
  shape_value(const int dof,
              const int q_point_no,
              const int direction,
              const int cell_no) const;

  const VectorizedArray<Number> &
  shape_value_face(const int dof,
                   const int face_no,
                   const int direction,
                   const int cell_no) const;

  const VectorizedArray<Number> &
  shape_gradient(const int dof,
                 const int q_point_no,
                 const int direction,
                 const int cell_no) const;
  ...
  patch_action(
    const FDEvaluation<dim, fe_degree_ansatz, n_q_points_ansatz, Number>
                   &eval_ansatz,
...
private:
  ...
  VectorizedArray<Number> &
  get_JxW_impl(const int q_point_no,
               const int direction,
               const int cell_no) const;
  ...
  void
  submit_cell_matrix(Table<2, VectorizedArray<Number>>       &subdomain_matrix,
                     ...
                     const TPSS::PatchLocalTensorHelper<dim>
                       *patch_dof_tensor_ansatz = nullptr) const;
   ...
   */
  std::array<std::bitset<macro_size>, GeometryInfo<dim>::faces_per_cell>
    at_boundary_masks;

  /**
   * Boundary id for each face of the macro patch.
   */
  std::array<std::array<types::boundary_id, macro_size>,
...
inline FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::FDEvaluation(
  ...
  , n_subdomains(
      sd_handler_in.get_patch_info().subdomain_partition_data.n_subdomains())
  ...
  , n_cells_per_direction(
      TPSS::UniversalInfo<dim>::n_cells_per_direction(patch_variant))
  ...
  for (auto d = 0U; d < dim; ++d)
    {
      ...
      AssertDimension(n_dofs_per_cell_1d(d),
                      get_shape_data(d, component).fe_degree + 1);
      /// static variables fe_order and n_q_points_1d_static have to define the
      /// maximum number for any dimension
      AssertIndexRange(n_dofs_per_cell_1d(d), fe_order + 1);
      AssertIndexRange(get_shape_data(d, component).quadrature.size(),
                       n_q_points_1d_static + 1);
      AssertIndexRange(get_shape_data(d, component).fe_degree + 1,
                       fe_order + 1);
  ...
  for (auto d = 0U; d < dim; ++d)
    {
      ...
      AssertIndexRange(static_cast<int>(quadrature.size()),
                       n_q_points_1d_static + 1);
      ...
      /// Extract one-dimensional shape function values on cell and face
      /// quadratures
      auto values_in_begin = shape_data.shape_values.begin();
      auto values_begin    = &(shape_value_impl(0, 0, d, 0));
      std::copy_n(values_in_begin,
                  fe_order * n_q_points_1d_static,
                  values_begin);
      for (const auto face_no : {0, 1})
        {
          auto values_face_in_begin =
            shape_data.shape_data_on_face[face_no].begin();
...
template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline void
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::reinit(
  ...
  if (TPSS::DoFLayout::Q == get_dof_layout())
    {
      ...
      std::copy_n(boundary_ids_in.cbegin(),
                  GeometryInfo<dim>::faces_per_cell,
                  boundary_ids.begin());
    }
...
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::malloc_fedata()
{
  ...
  constexpr unsigned int gradients_length_face =
    fe_order * 2 * n_cells_per_direction * dim;
  constexpr unsigned int hessians_length =
    n_q_points_1d_static * fe_order * n_cells_per_direction * dim;
  constexpr unsigned int hessians_length_face =
    fe_order * 2 * n_cells_per_direction * dim;
  constexpr unsigned int JxWs_length =
    n_q_points_1d_static * n_cells_per_direction * dim;
  constexpr unsigned int size_to_be_allocated =
    scratch_pad_length + unit_weights_length + values_length +
    values_length_face + gradients_length + gradients_length_face +
    hessians_length + hessians_length_face + JxWs_length; // total size
  ...
  for (unsigned int d = 0; d < dim; ++d)
    this->gradients[d] =
      this->values_face + values_length_face +
      n_q_points_1d_static * fe_order * n_cells_per_direction * d;
  for (unsigned int d = 0; d < dim; ++d)
    this->gradients_face[d] = this->gradients[0] + gradients_length +
                              fe_order * 2 * n_cells_per_direction * d;
  for (unsigned int d = 0; d < dim; ++d)
    this->hessians[d] =
      this->gradients_face[0] + gradients_length_face +
      n_q_points_1d_static * fe_order * n_cells_per_direction * d;
  for (unsigned int d = 0; d < dim; ++d)
    this->hessians_face[d] = this->hessians[0] + hessians_length +
                             fe_order * 2 * n_cells_per_direction * d;
  ...
  AssertDimension(std::distance(this->scratch_fedata->begin(),
                                this->scratch_pad_end),
...
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::acquire_scratch_chunk(
  const std::size_t size)
{
  auto &&chunk = make_array_view(this->scratch_pad_remainder,
                                 (this->scratch_pad_remainder += size));
  ...
  const ArrayView<VectorizedArray<Number>> &matrix) const
{
  ...
  CellAssembler::template assemble<false>(
    make_array_view(this->values,
                    this->values + fe_order * n_q_points_1d_static),
    make_array_view(this->values,
                    this->values + fe_order * n_q_points_1d_static),
    make_array_view(this->q_weights_unit,
                    this->q_weights_unit + n_q_points_1d_static),
    matrix);
}
...
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::get_average_factor(
  const int direction,
  const int cell_no,
  const int face_no) const
{
...
template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline std::bitset<
  FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::macro_size>
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::get_boundary_mask(
  const int direction,
  const int cell_no,
  const int face_no) const
{
  Assert(patch_id != numbers::invalid_unsigned_int,
         ExcMessage(
           "FDEvaluation is not initialized. Did you miss to call reinit()?"));
...
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::get_h(
  const int direction,
  const int cell_no) const
{
  Assert(patch_id != numbers::invalid_unsigned_int,
...
template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline const VectorizedArray<Number> &
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::get_h_impl(
  const int direction,
  const int cell_no) const
{
  AssertIndexRange(cell_no, static_cast<int>(n_cells_per_direction));
...
template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline const VectorizedArray<Number> &
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::get_JxW(
  const int q_point_no,
  const int direction,
  const int cell_no) const
{
...
template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline VectorizedArray<Number> &
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::get_JxW_impl(
  const int q_point_no,
  const int direction,
  const int cell_no) const
{
...
template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline const VectorizedArray<Number> &
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::get_q_weight(
  const int q_point_no,
...
template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline VectorizedArray<Number> &
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::get_q_weight_impl(
  const int q_point_no,
  const int direction) const
{
  AssertIndexRange(q_point_no, n_q_points_1d(direction));
  AssertIndexRange(direction, dim);
  return *(this->q_weights_unit + q_point_no +
           direction * n_q_points_1d_static);
}


template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline VectorizedArray<Number>
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::get_normal(
  const int face_no) const
{
  Assert(patch_id != numbers::invalid_unsigned_int,
         ExcMessage(
...
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::get_normal_vector(
  const int face_no,
  const int direction) const
{
  Assert(patch_id != numbers::invalid_unsigned_int,
...
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::
  get_constrained_dof_indices(const unsigned int lane) const
{
...
template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline const internal::MatrixFreeFunctions::UnivariateShapeData<
  VectorizedArray<Number>> &
...
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::get_subdomain_handler()
  const
{
  return sd_handler;
}
...
template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline unsigned int
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::n_cells_1d(
...
template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline const VectorizedArray<Number> &
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::shape_value(
  const int dof,
  const int q_point_no,
  const int direction,
  const int cell_no) const
...
template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline VectorizedArray<Number> &
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::shape_value_impl(
  const int dof,
  const int q_point_no,
  const int direction,
  const int cell_no) const
{
  ...
  return *(this->values + q_point_no + dof * n_q_points_1d_static +
           direction * n_values_1d);
}


template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline const VectorizedArray<Number> &
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::shape_value_face(
  const int dof,
  const int face_no,
  const int direction,
  const int cell_no) const
...
template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline VectorizedArray<Number> &
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::shape_value_face_impl(
  const int dof,
  const int face_no,
  const int direction,
  const int cell_no) const
{
  ...
  return *(this->values_face + dof + fe_order * face_no +
           direction * n_values_on_faces_1d);
}


template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline const VectorizedArray<Number> &
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::shape_gradient(
  const int dof,
  const int q_point_no,
  const int direction,
  const int cell_no) const
...
template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline VectorizedArray<Number> &
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::shape_gradient_impl(
  const int dof,
  const int q_point_no,
  const int direction,
  const int cell_no) const
{
  ...
  return *(this->gradients[direction] + q_point_no +
           dof * n_q_points_1d_static + cell_no * n_dofs_per_cell_1d_static);
}


template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline const VectorizedArray<Number> &
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::shape_gradient_face(
  const int dof,
  const int face_no,
  const int direction,
  const int cell_no) const
...
template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline const VectorizedArray<Number> &
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::shape_hessian(
  const int dof,
  const int q_point_no,
  const int direction,
  const int cell_no) const
...
template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline VectorizedArray<Number> &
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::shape_hessian_impl(
  const int dof,
  const int q_point_no,
  const int direction,
  const int cell_no) const
...
template <int dim, int fe_degree, int n_q_points_1d_, typename Number>
inline const VectorizedArray<Number> &
FDEvaluation<dim, fe_degree, n_q_points_1d_, Number>::shape_hessian_face(
  const int dof,
  const int face_no,
  const int direction,
  const int cell_no) const
