...
namespace TPSS
{
  ...
  private:
    ...
    const typename MatrixFreeConnect<dim, Number>::DoFInfoLocal
      *const dof_info_local;
  };
    ...
    , dof_info_local(&(mf_connect_in.get_dof_info_local(dofh_index)))
  {
    ...
    AssertThrow(mf_connect->patch_info,
                ExcMessage("mf_connect is not initialized."));
    ...
    if (!dof_info_local->is_intentionally_uninitialized) // TODO !!!
      AssertDimension(this->patch_info->n_cells_plain() + 1,
                      dof_info_local->dof_starts.size());
    AssertDimension(this->patch_info->n_cells_plain(),
                    mf_connect->batch_and_lane_index_pairs.size());
  }


  template <int dim, typename Number>
  inline std::pair<unsigned int, unsigned int>
  PatchMFWorker<dim, Number>::get_batch_and_lane_index(
  ...
  template <int dim, typename Number>
  inline std::vector<std::array<std::pair<unsigned int, unsigned int>,
                                PatchMFWorker<dim, Number>::macro_size>>
  PatchMFWorker<dim, Number>::get_batch_collection(unsigned int patch_id) const
  {
    AssertIndexRange(patch_id,
                     this->patch_info->subdomain_partition_data.n_subdomains());
    const auto n_lanes_filled = this->n_lanes_filled(patch_id);

    std::vector<std::array<std::pair<unsigned int, unsigned int>, macro_size>>
      collection(this->n_cells_per_subdomain());
    for (auto cell_no = 0U; cell_no < collection.size(); ++cell_no)
      {
        for (auto lane = 0U; lane < n_lanes_filled; ++lane)
          collection[cell_no][lane] =
            get_batch_and_lane_index(patch_id, cell_no, lane);
        for (auto lane = n_lanes_filled; lane < macro_size; ++lane)
          collection[cell_no][lane] =
            get_batch_and_lane_index(patch_id, cell_no, 0);
      }
    return collection;
  }
  ...
  PatchMFWorker<dim, Number>::get_local_cell_and_patch_dof_indices(
    const unsigned int patch_id,
    const unsigned int cell_no,
    const unsigned int lane) const
  {
    ...
    AssertIndexRange(dof_start + n_dofs,
                     dof_info_local->cell_and_patch_dof_indices.size() + 1);
    const auto start =
      dof_info_local->cell_and_patch_dof_indices.data() + dof_start;
    return ArrayView<const std::pair<unsigned int, unsigned int>>(start,
                                                                  n_dofs);
  }
  ...
  template <int dim, typename Number>
  inline unsigned int
  PatchMFWorker<dim, Number>::n_dofs_per_cell_on_patch(
    const unsigned int patch_id,
    const unsigned int cell_no,
    const unsigned int lane) const
  {
