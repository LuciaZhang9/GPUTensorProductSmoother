...
struct InverseTable
{
  ...
  reinit(const Table<2, Number> &matrix_in,
         const AdditionalData   &additional_data_in = AdditionalData{})
  {
    Assert(matrix_in.size(0) == matrix_in.size(1),
           ExcMessage("Matrix is not square."));
    clear();

    auto inverses = std::make_shared<
      std::array<LAPACKFullMatrix<scalar_value_type>, macro_size>>();
    ...
    for (auto lane = 0U; lane < macro_size; ++lane)
      {
        ...
        if (additional_data_in.kernel_size != numbers::invalid_unsigned_int)
          inverse.compute_inverse_svd_with_kernel(
            additional_data_in.kernel_size);
        else
          inverse.compute_inverse_svd(additional_data_in.threshold);
      }
    /// ALTERNATIVE: FullMatrix
    // auto inverses =
    // std::make_shared<std::array<FullMatrix<scalar_value_type>,
    // macro_size>>(); const unsigned int n_rows = matrix_in.size(0); for(auto
    // lane = 0U; lane < macro_size; ++lane)
    // {
    //   auto & inverse = (*inverses)[lane];
  ...
  {
    Vector<scalar_value_type> dst(dst_view.size()),
      src(src_view.cbegin(), src_view.cend());
  ...
  {
    Vector<scalar_value_type> dst_lane(dst_view.size()),
      src_lane(src_view.size());
    for (auto lane = 0U; lane < macro_size; ++lane)
      {
  ...
  std::shared_ptr<
    const std::array<LAPACKFullMatrix<scalar_value_type>, macro_size>>
    inverses;
  /// ALTERNATIVE: FullMatrix
  // std::shared_ptr<const std::array<FullMatrix<scalar_value_type>,
  // macro_size>> inverses;
};
...
public:
  ...
  {
    inverse_matrix =
      std::make_shared<InverseTable<Number>>(matrix, additional_data);
  }
  ...
  apply_inverse(const ArrayView<Number>       &dst_view,
                const ArrayView<const Number> &src_view) const
  {
    AssertThrow(inverse_matrix,
                ExcMessage(
                  "Have you called invert() after modifying entries?"));
  ...
  {
    AssertIndexRange(row_start +
                       (fill_transpose ? other.size(1) : other.size(0)) - 1,
                     m());
    AssertIndexRange(column_start +
                       (fill_transpose ? other.size(0) : other.size(1)) - 1,
                     n());
...
public:
  ...
  DiagonalMatrixWrap(
    const AlignedVector<Number>     &diagonal_in,
           ...
           {static_cast<std::size_t>(-1), static_cast<std::size_t>(-1)})
  {
    ...
    const bool treat_as_square = m_in == static_cast<std::size_t>(-1) &&
                                 n_in == static_cast<std::size_t>(-1);
    size = treat_as_square ? std::array<std::size_t, 2>{diagonal_in.size(),
                                                        diagonal_in.size()} :
                             size_in;

    const auto [m, n] = size;
    (void)m, (void)n;
    Assert(diagonal_in.size() == m || diagonal_in.size() == n,
           ExcMessage("Mismatching size."));
        ...
        for (auto ii = 0U; ii < mm; ++ii)
          for (auto i = 0U; i < m; ++i)
            dst_view[transpose ? i + i * m : ii + ii * mm] +=
              diagonal[i + ii * m] *
              src_view[transpose ? ii + ii * mm : i + i * m];
      }

    else
      {
        /// if the matrix isn't squared the diagonal has either less elements
        /// than rows or than columns, thus, the loop is bound by
        /// diagonal.size()
...
struct PatchMatrix
{
  ...
  static constexpr unsigned int macro_size =
    VectorizedArray<scalar_value_type>::size();
...
struct ConstrainedMatrix
{
        ...
        const ArrayView<const Number> &src_view) const
  {
    ...
    const ArrayView<Number> constrained_src_view(tmp_array.begin(),
                                                 src_view.size());
