...
namespace TPSS
{
  namespace internal
  {
    ...
    template <typename Number>
    const Number
    local_element(const LinearAlgebra::distributed::Vector<Number> &vec,
    ...
    template <typename Number>
    Number &
    local_element(LinearAlgebra::distributed::Vector<Number> &vec,
                  const unsigned int                          dof_index)
    {
      return vec.local_element(dof_index);
    }
   ...
   * lexicographically.
   *
   * Mappings between cell local dof indices (subject to lexicographical
   * ordering) for each cell within a patch and the patch relevant dof indices
   * are required. Obviously, for "DGQ cell patches" it is the identity. For DGQ
   * vertex patches we have to take into account that each patch has 2 cells in
   * each spatial coordinate. For Q vertex patches we additionally have to treat
   * the dofs on (logical) cell boundaries belonging to more than one cell.
   *
   * Nevertheless, we restrict ourselves to patches with tensor product
   * structure of cells. The helper class PatchLocalTensorHelper provides all
   ...
   * subdomain_handler. To be precise, access to global dof indices is provided
   * by PatchDoFWorker.
   */
  ...
  public:
    ...
    /**
     * Returns the set of degrees of freedom (as proc-local index) on the
     * current patch with lexicographical ordering (if possible, e.g. DGP
     * polynomials have no such ordering).
     */
    ArrayView<const unsigned int>
    get_dof_indices(const unsigned int lane) const;

    /**
     * Same as above, but this time only a subset of degrees of freedom for
     * vector
     * component @p component is returned.
     */
    ArrayView<const unsigned int>
    get_dof_indices(const unsigned int lane,
    ...
    template <typename VectorType>
    void
    gather_add(AlignedVector<VectorizedArray<Number>> &dst,
    ...
    template <typename VectorType>
    void
    gather_add(const ArrayView<VectorizedArray<Number>> dst,
               const VectorType                        &src) const;
     ...
     * all dofs that are not restricted. This restricted prolongation is used
     * for restricted additive Schwarz methods (RAS) defining a partition of
     * unity at the algebraic level.
     */
    template <typename VectorType>
    void
    rscatter_add(VectorType                                   &dst,
                 const AlignedVector<VectorizedArray<Number>> &src) const;
  ...
  private:
    // /**
    //  * Read access to patch-local dof indices of cell @cell_no. The returned
    //  array
    ...
    void
    scatter_add_impl(VectorType                                    &dst,
                     const ArrayView<const VectorizedArray<Number>> src) const;
    ...
    /**
     * A bijective map between patch local dof indices (position index) and
    ...
    /**
     * Current patch id identifying a unique patch given by the underlying
     * PatchInfo.
     */
   ...
   * block is provided by a PatchTransfer object, set to the according dof
   * handler.
   *
   * TESTED This class is used with blocks associated to different finite
   * elements in tests regarding the (local) Stokes integrators.
   */
  template <int dim, typename Number>
  ...
  public:
    ...
    PatchTransferBlock(
      const std::vector<TPSS::DoFInfo<dim, Number>> &dof_infos);
     ...
     * ordering) for each block. These sets of dof indices are juxtaposed block
     * by block.
     */
    ArrayView<const unsigned int>
    get_dof_indices(const unsigned int lane) const;
     ...
     */
    ArrayView<const unsigned int>
    get_dof_indices(const unsigned int block_index,
    ...
    /**
     * For each block we use the according PatchTransfer to initialize the
     * global dof vector. For more details see
     * PatchTransfer::initialize_dof_vector().
     */
    void
    initialize_dof_vector(
    ...
    template <typename BlockVectorType>
    void
    scatter_add(BlockVectorType                              &dst,
                const AlignedVector<VectorizedArray<Number>> &src) const;
    ...
    template <typename BlockVectorType>
    void
    rscatter_add(BlockVectorType                              &dst,
                 const AlignedVector<VectorizedArray<Number>> &src) const;
  ...
  private:
    ...
    void
    scatter_add_impl(BlockVectorType                              &dst,
                     const AlignedVector<VectorizedArray<Number>> &src) const;
  ...
  //   operator()(const TPSS::PatchVariant patch_variant, const bool dg = true)
  //   const
  //   {
  ...
  //     const std::size_t         n_dofs_per_subdomain =
  ...
  //   {
  //     // LAMBDA uniquely maps the (cell index, cell number) pair to the patch
  //     index const auto cell_to_patch_index = [](const unsigned int cindex,
  ...
  //       if(dim == 3)
  //       {
  //         const unsigned int iz = cindex / static_cast<unsigned
  //         int>(Utilities::pow(fe_order, 2)); const unsigned int remainder_y =
  //           cindex % static_cast<unsigned int>(Utilities::pow(fe_order, 2));
  //         const unsigned int iy = static_cast<unsigned int>(remainder_y /
  //         fe_order); const unsigned int ix = static_cast<unsigned
  //         int>(remainder_y % fe_order); patch_index += iz * 4 * (fe_order *
  //         fe_order) + iy * 2 * fe_order + ix;
  //         //      std::cout << "iz,ry,iy,pi " << iz << " " << remainder_y <<
  //         " " << iy << " " <<
  //         //      patch_index << std::endl ;
  //       }
  //       if(dim == 2)
  //       {
  //         const unsigned int iy = static_cast<unsigned int>(cindex /
  //         fe_order); const unsigned int ix = static_cast<unsigned int>(cindex
  //         % fe_order); patch_index += ix + iy * 2 * fe_order;
  //       }
  //       if(dim == 1)
  //         patch_index += cindex;
  ...
  //       for(unsigned int dof = 0; dof < n_dofs_per_cell_static; ++dof,
  //       ++patch_index)
  ...
  inline PatchTransfer<dim, Number>::PatchTransfer(
    ...
    : level([&dof_info_in]() {
      AssertThrow(dof_info_in.patch_info,
                  ExcMessage("dof_info_in is not initialized."));
      return dof_info_in.patch_info->get_additional_data().level;
    }())
    , n_subdomains(
        dof_info_in.patch_info->subdomain_partition_data.n_subdomains())
  ...
  {
    AssertThrow(dof_layout != DoFLayout::invalid,
  ...
  template <int dim, typename Number>
  inline void
  PatchTransfer<dim, Number>::reinit_local_vector(
  ...
  template <int dim, typename Number>
  inline ArrayView<const unsigned int>
  PatchTransfer<dim, Number>::get_dof_indices(
    ...
    const unsigned int component) const
  {
    ...
    if (caching_strategy == TPSS::CachingStrategy::Cached)
      return patch_dof_worker.get_dof_indices_on_patch(patch_id,
                                                       lane,
                                                       component);
    else
      AssertThrow(false,
                  ExcMessage("Only implemented for CachingStrategy::Cached."));
  ...
  template <int dim, typename Number>
  inline void
  PatchTransfer<dim, Number>::fill_global_dof_indices(
    const unsigned int patch_id)
  {
    AssertIndexRange(patch_id, n_subdomains);
    for (auto lane = 0U; lane < patch_dof_worker.n_lanes_filled(patch_id);
         ++lane)
      {
  ...
  inline PatchTransferBlock<dim, Number>::PatchTransferBlock(
    ...
    : PatchTransferBlock<dim, Number>([&subdomain_handler_in]() {
      ...
      for (auto dofh_index = 0U;
           dofh_index < subdomain_handler_in.n_dof_handlers();
           ++dofh_index)
        dof_infos.emplace_back(
          &(subdomain_handler_in.get_dof_info(dofh_index)));
      return dof_infos;
    }())
  {}
  ...
  template <int dim, typename Number>
  const PatchDoFWorker<dim, Number> &
  PatchTransferBlock<dim, Number>::get_patch_dof_worker(
  ...
  PatchTransferBlock<dim, Number>::get_patch_transfer(
    const unsigned int block_index) const
  {
    AssertIndexRange(block_index, n_blocks);
  ...
  template <int dim, typename Number>
  ArrayView<const unsigned int>
  PatchTransferBlock<dim, Number>::get_dof_indices(
    const unsigned int lane) const
  {
    /// Currently, each block has its own dof cache, such that there is no
    /// contiguous dof index cache between blocks. Therefore, it is inevitable
    /// to copy the sets of indices block-wise and return an ArrayView based on
    /// this intermediate cache.
    Assert(patch_id != numbers::invalid_unsigned_int, ExcNotInitialized());
    ...
    for (auto b = 0U; b < n_blocks; ++b)
      {
        const auto get_dof_indices_per_block = [&]() {
          ...
          const auto  caching_strategy = patch_dof_worker.get_dof_info()
                                          .get_additional_data()
                                          .caching_strategy;
    ...
    return ArrayView<const unsigned int>(dof_indices_cached.data(),
                                         dof_indices_cached.size());
  }
  ...
  template <int dim, typename Number>
  inline unsigned int
  PatchTransferBlock<dim, Number>::n_dofs_per_patch(
    const unsigned int block_index) const
  {
    AssertIndexRange(block_index, n_blocks);
  ...
  template <typename BlockVectorType>
  inline void
  PatchTransferBlock<dim, Number>::initialize_dof_vector_impl(
  ...
  PatchTransferBlock<dim, Number>::initialize_dof_vector(
    LinearAlgebra::distributed::BlockVector<Number> &vec) const
  {
    initialize_dof_vector_impl<LinearAlgebra::distributed::BlockVector<Number>>(
      vec);
  }
