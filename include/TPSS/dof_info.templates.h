...
namespace TPSS
{
  ...
  {
    Assert(patch_info_in->get_internal_data()->level !=
  ...
  DoFInfo<dim, Number>::initialize_impl()
  {
    ...
    /// (including ghost cells)
    {
      ...
      const auto &is_ghost_on_level_impl = [](const auto &cell,
                                              const auto  my_subdomain_id) {
      ...
      /// LAMBDA checks if cell has a neighbor being a ghost cell on the current
      /// level
      const auto &has_ghost_neighbor_on_level = [&](const auto &cell) {
        for (auto face_no = 0U; face_no < GeometryInfo<dim>::faces_per_cell;
             ++face_no)
          if (cell.neighbor_level(face_no) ==
              static_cast<int>(additional_data.level))
      ...
      const auto               n_subdomains =
        patch_worker.get_partition_data().n_subdomains();
            ...
            for (auto lane = 0U; lane < n_lanes_filled; ++lane)
              {
                const auto cell_position =
                  patch_worker.get_cell_position(patch, cell_no, lane);
                ...
                cell_index_to_cell_position[cell_index].push_back(
                  cell_position);
              }
        }
      ...
      for (const auto &[cell_index, cell_positions] :
           cell_index_to_cell_position)
        {
          const auto &cell =
            get_level_dof_accessor_impl(cell_index, additional_data.level);
      ...
      /// First, sort and compress duplicates of ghosted global dof indices.
      /// Then, fill ghost index set.
      ...
      ghost_dof_indices.add_indices(dof_indices_on_ghosts.begin(),
                                    end_without_duplicates);
      ghost_dof_indices.compress();

      /// Initialize vector partitioner based on locally owned and ghosted dof
      /// indices.
      ...
      if (!is_mpi_parallel)
        AssertDimension(dof_handler->n_dofs(additional_data.level),
      ...
      std::transform(global_dof_indices_cellwise.cbegin(),
                     ...
                     [&](const auto global_dof_index) {
                       return vector_partitioner->global_to_local(
                         global_dof_index);
                     });
    }
    ...
    if (additional_data.caching_strategy == TPSS::CachingStrategy::Cached)
      {
        ...
        /// patch-wise (to be precise, we use the process local dof index to
        /// save memory).
        ...
        for (auto patch_id = 0U; patch_id < n_subdomains; ++patch_id)
          {
            start_of_dof_indices_patchwise.emplace_back(
              dof_indices_patchwise.size());
            for (auto lane = 0U;
  ...
  DoFInfo<dim, Number>::compute_restricted_dofs_impl()
  {
    ...
    AssertThrow(n_components == 1U,
                ExcMessage("TODO more than scalar elements"));
    const auto patch_variant = patch_info->get_additional_data().patch_variant;
    const auto smoother_variant =
      patch_info->get_additional_data().smoother_variant;
    ...
    std::map<unsigned int, std::vector<std::array<unsigned int, 5>>>
                       dof_indices_to_weights;
    ...
    for (auto patch_id = 0U; patch_id < n_subdomains; ++patch_id)
      {
        for (auto lane = 0U; lane < patch_dof_worker.n_lanes_filled(patch_id);
             ++lane)
          {
            ...
            const auto comp_weight =
              [&](const unsigned int index) -> std::array<unsigned int, 5> {
              AssertDimension(dof_tensor_inner.n_flat(),
                              dof_indices_on_patch.size());
    ...
    const auto get_dof_position_patchwise = [&](const unsigned int patch_id,
      ...
      const auto [dof_start, n_dofs_at_comp] =
        patch_dof_worker.get_dof_start_and_quantity_on_patch(patch_id,
                                                             lane,
                                                             component);
    ...
    for (auto &[dof_index, weights] : dof_indices_to_weights)
      {
        ...
        (void)ignore;
        const auto dof_position =
          get_dof_position_patchwise(patch_id, lane, comp, dof_no);
