...
struct SchwarzSmootherData
{
  ...
  {
    std::string str_patch_variant[]    = {"Undefined",
                                          "Cell-Based",
                                          "Vertex Patch"};
    std::string str_smoother_variant[] = {"Undefined",
                                          "Additive",
                                          "Multiplicative"};
    std::string str_schwarz_operator =
      str_smoother_variant[(int)smoother_variant] + " " +
      str_patch_variant[(int)patch_variant];

    print_parameter(pcout, "Schwarz operator", str_schwarz_operator);
    // print_parameter(pcout, "Number of smoothing steps",
    // number_of_smoothing_steps);
    ...
    print_parameter(pcout,
                    "Number of quad points (surrogate)",
                    n_q_points_surrogate);
    print_parameter(pcout,
                    "Normalize surrogate patches",
                    normalize_surrogate_patch);
    print_parameter(pcout, "Compute arc length (surrogate)", use_arc_length);

    print_parameter(pcout,
                    "Caching strategy",
                    TPSS::str_caching_strategy(caching_strategy));
  }
  ...
  const SchwarzSmootherData                              &schwarz_smoother_data)
{
  ...
  additional_data.compute_ras_boolean_weights =
    schwarz_smoother_data.use_ras_boolean_weights;
  /// Surrogates
  additional_data.n_q_points_surrogate =
    schwarz_smoother_data.n_q_points_surrogate;
  additional_data.normalize_surrogate_patch =
    schwarz_smoother_data.normalize_surrogate_patch;
