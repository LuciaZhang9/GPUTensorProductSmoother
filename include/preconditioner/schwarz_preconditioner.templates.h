...
SchwarzPreconditioner<dim, OperatorType, VectorType, MatrixType>::initialize(
  const std::shared_ptr<const SubdomainHandler<
    ...
    typename SchwarzPreconditioner<dim, OperatorType, VectorType, MatrixType>::
      value_type>>      subdomain_handler_in,
  ...
  const AdditionalData &additional_data_in)
{
  ...
  const unsigned int n_colors =
    subdomain_handler_in->get_partition_data().n_colors();
  const unsigned int n_colors_max =
    Utilities::MPI::max(n_colors, MPI_COMM_WORLD);
  const unsigned int n_colors_min =
    Utilities::MPI::min(n_colors, MPI_COMM_WORLD);
  (void)n_colors_max, (void)n_colors_min;
  Assert(n_colors_max == n_colors_min,
         ExcMessage("Not the same number of colors on each proc."));
  ...
  Assert(patch_variant == TPSS::PatchVariant::vertex ||
           patch_variant == TPSS::PatchVariant::cell,
  ...
  const AdditionalData        &additional_data_in)
{
  ...
  Assert(additional_data.relaxation > 0.,
         ExcMessage("Invalid relaxation factor."));
  Assert(patch_variant == TPSS::PatchVariant::vertex ||
           patch_variant == TPSS::PatchVariant::cell,
  ...
  compute_inverses()
{
  ...
  const auto make_assembling =
    [this](const SubdomainHandler<dim, value_type>     &data,
           std::vector<MatrixType>                     &inverses,
           const OperatorType                          &operator_,
           const std::pair<unsigned int, unsigned int> &range) {
      linear_operator->assemble_subspace_inverses(data,
                                                  inverses,
                                                  operator_,
                                                  range);
    };

  // *** loop over all patches in parallel (regardless of color and
  // constitution)
  ...
  compute_ras_weights()
{
  ...
  const auto count_appearances =
    [this](const SubdomainHandler<dim, value_type>     &data,
      ...
      (void)dummy2;
      auto transfer =
        std::make_shared<typename OperatorType::transfer_type>(data);
      AlignedVector<VectorizedArray<value_type>> local_one;
      for (auto patch_id = subdomain_range.first;
           patch_id < subdomain_range.second;
           ++patch_id)
        {
  ...
  for (unsigned int color = 0; color < n_colors; ++color)
    {
      ...
      subdomain_handler->template loop<const VectorType, VectorType>(
        std::ref(count_appearances), ras_weights, ras_weights, color);
    ...
    const LinearAlgebra::distributed::Vector<value_type> &vec) const
{
  ...
  const bool  is_compatible =
    vec.partitioners_are_globally_compatible(*partitioner);
  ...
  for (unsigned int b = 0; b < partitioners.size(); ++b)
    {
      is_compatible =
        is_compatible &&
        vec.block(b).partitioners_are_globally_compatible(*(partitioners[b]));
    }
  return is_compatible;
}
  ...
  if (vec.n_blocks() == partitioners.size())
    {
      ...
      for (unsigned int b = 0; b < vec.n_blocks(); ++b)
        is_compatible &=
          vec.block(b).partitioners_are_compatible(*(partitioners[b]));
      if (is_compatible)
...
SchwarzPreconditioner<dim, OperatorType, VectorType, MatrixType>::
  preprocess_solution_and_residual(
    LinearAlgebra::distributed::Vector<
      typename SchwarzPreconditioner<dim,
                                     OperatorType,
                                     VectorType,
                                     MatrixType>::value_type> &solution_in,
    const LinearAlgebra::distributed::Vector<
      typename SchwarzPreconditioner<dim,
                                     OperatorType,
                                     VectorType,
                                     MatrixType>::value_type> &residual_in)
    const
{
...
SchwarzPreconditioner<dim, OperatorType, VectorType, MatrixType>::
  preprocess_solution_and_residual(
    Vector<typename SchwarzPreconditioner<dim,
                                          OperatorType,
                                          VectorType,
                                          MatrixType>::value_type> &solution_in,
    const Vector<typename SchwarzPreconditioner<dim,
                                                OperatorType,
                                                VectorType,
...
SchwarzPreconditioner<dim, OperatorType, VectorType, MatrixType>::
  preprocess_solution_and_residual(
    BlockVector<typename SchwarzPreconditioner<dim,
                                               OperatorType,
                                               VectorType,
                                               MatrixType>::value_type>
      &solution_in,
    const BlockVector<typename SchwarzPreconditioner<dim,
                                                     OperatorType,
                                                     VectorType,
                                                     MatrixType>::value_type>
      &residual_in) const
...
void
SchwarzPreconditioner<dim, OperatorType, VectorType, MatrixType>::
  postprocess_solution(OtherVectorType &, const OtherVectorType &) const
{
  AssertThrow(false, ExcMessage("VectorType is not supported."));
}
  ...
  postprocess_solution(
    LinearAlgebra::distributed::Vector<
      typename SchwarzPreconditioner<dim,
                                     OperatorType,
                                     VectorType,
                                     MatrixType>::value_type> &solution_dst,
    const LinearAlgebra::distributed::Vector<
      typename SchwarzPreconditioner<dim,
                                     OperatorType,
                                     VectorType,
                                     MatrixType>::value_type> &solution_src)
    const
{
...
void
SchwarzPreconditioner<dim, OperatorType, VectorType, MatrixType>::
  apply_local_solvers(VectorType        &solution_in,
                      const VectorType  &residual_in,
                      const unsigned int color) const
{
  // *** pre-process solution and residual vectors (if needed)
  auto [solution, residual] =
    preprocess_solution_and_residual(solution_in, residual_in);
  ...
  const bool use_ras_boolean_weights =
    additional_data.use_ras_boolean_weights && is_additive;
    ...
    const auto apply_inverses_add =
      [&](const SubdomainHandler<dim, value_type>     &data,
        ...
        for (auto patch_id = subdomain_range.first;
             ...
             ++patch_id)
          {
            /// restrict global to local residual and initialize local solution
            /// u_j
            transfer->reinit(patch_id);
            ...
            const ArrayView<const VectorizedArray<value_type>>
              local_residual_view =
            ...
            if (additional_data.relaxation != 1.)
              for (auto &elem : local_solution)
                elem *=
                  make_vectorized_array<value_type>(additional_data.relaxation);
            ...
            /// in case of standard RAS the prolongation operator R_j^T
            /// satisfies a partition of unity
  ...
  subdomain_handler->template loop<const VectorType, VectorType>(
    std::ref(apply_inverses_add), *solution, *residual, color);
...
void
SchwarzPreconditioner<dim, OperatorType, VectorType, MatrixType>::
  additive_schwarz_operation(VectorType &dst, const VectorType &src) const
{
  AssertThrow(!transpose,
              ExcMessage("TODO transpose operation is not implemented."));

  /// we separate the sum over subdomains with respect to colors avoiding race
  /// conditions
  const auto &partition_data = subdomain_handler->get_partition_data();
  const auto  n_colors       = partition_data.n_colors();
                                   ...
                                   const VectorType &rhs) const
{
  ...
  const std::vector<unsigned int> color_sequence =
    get_color_sequence(transpose);
  ...
  /// apply inverses of first color (no update of residual needed since the
