...
public:
  using value_type =
    typename ExtractScalarType<typename MatrixType::value_type>::type;
  ...
  /*
   * Computes the local solvers (local inverse problems) based on the domain
   * decomposition given by
   * @p subdomain_handler and the finite element discretization represented by @p linear
   * operator. In addition, the linear operator must describe the (global)
   * forward problem required to compute residuals.
   */
  void
  initialize(const std::shared_ptr<const SubdomainHandler<dim, value_type>>
                                           subdomain_handler,
   ...
   * example, if another Schwarz preconditioner with a different color sequence,
   * e.g. due to symmetry reasons, is needed but the set of local solvers is the
   * same. This type of
   ...
   */
  void
  initialize(const OperatorType   &linear_operator,
             const AdditionalData &additional_data);
  ...
  bool
  is_shallow_copyable(
    const typename SubdomainHandler<dim, value_type>::AdditionalData
      &other_data) const
  {
    ...
    // TODO gathering ?
    is_copyable &=
      sdhandler.n_q_points_surrogate == other_data.n_q_points_surrogate;
    is_copyable &= sdhandler.normalize_surrogate_patch ==
                   other_data.normalize_surrogate_patch;
...
private:
  /**
   * Implementing the application of the additive Schwarz preconditioner
   * A^{-1}_{ad}:
   *
   *    x = A^{-1}_{ad} b
   *
  ...
  /**
   * Computes the action: u_new = u_prev + omega * sum_{j \in J(color)} R_j^T
   * A_j^{-1} R_j r
   *
   * u_new    : new solution vector after one colored smoothing step of r, which
   * is returned by
   *            reference @p solution
   *
   * u_prev   : previous solution vector used as initial guess of smoothing
   * step, which is passed by
   *            reference @p solution
   *
   * J(color) : index set containing all subdomains with color @p color
   *
   * R_j      : transfer from global unknowns to local unknowns associated to
   * subspace j (restriction)
   *
   * A_j^{-1} : local solver of subproblem j which is stored in @p subdomain_to_inverse
   *
   * R_j^T    : transfer from local unknowns associated to subspace j to global
   * unknowns (prolongation)
   *
  ...
  preprocess_solution_and_residual_distributed(
  ...
  preprocess_solution_and_residual(
    ...
    const LinearAlgebra::distributed::BlockVector<value_type> &residual_in)
    const;
  ...
  preprocess_solution_and_residual(
    BlockVector<value_type>       &solution_in,
  ...
  template <typename OtherVectorType = VectorType>
  void
  postprocess_solution(OtherVectorType       &solution_dst,
                       const OtherVectorType &solution_src) const;
  ...
  void
  postprocess_solution(
    LinearAlgebra::distributed::BlockVector<value_type> &,
   ...
   */
  void
  postprocess_solution(BlockVector<value_type> &,
                       const BlockVector<value_type> &) const;
  ...
  /**
   * The sequence of colors for the smoothing step. Colors are defined by the
   * PartitionData of @p subdomain_handler and the ordering is determined in
   * terms of the AdditionalData.
   */
  ...
  initialize_ghosted_vector_if_needed(
    LinearAlgebra::distributed::Vector<value_type> &vec) const;
  ...
  /**
   * Implementing the application of the multiplicative Schwarz preconditioner
   * A^{-1}_{mu}:
   *
   *    x = A^{-1}_{mu} b
   *
  ...
  template <bool transpose = false>
  void
  multiplicative_schwarz_operation(VectorType       &solution,
                                   const VectorType &rhs) const;

  /**
   * Underlying infrastructure storing data with respect to the domain
   * decomposition, partitioning of patches, linking to MatrixFree
   * infrastructure, etc ...
   */
   ...
   * II inverse problem:
   *
   * Second, the linear operator is responsible of computing local solvers via
   * the method template<typename MatrixType, typename OperatorType> void
   *  assemble_subspace_inverses(const SubdomainHandler<dim, Number> & data,
   *                             std::vector<MatrixType> &             inverses,
   *                             const OperatorType & linear_operator, const
   * std::pair<unsigned int, unsigned int> subdomain_range) const
   */
  OperatorType *linear_operator = nullptr;

  /**
   * Local solvers stored as (inverse) matrices. The shared pointer allows
   * shallow copies.
   */
...
template <int dim, class OperatorType, typename VectorType, typename MatrixType>
struct SchwarzPreconditioner<dim, OperatorType, VectorType, MatrixType>::
  AdditionalData
{
  double relaxation              = 1.;
  bool   reverse                 = false;
...
template <int dim, class OperatorType, typename VectorType, typename MatrixType>
inline const typename SchwarzPreconditioner<dim,
                                            OperatorType,
                                            VectorType,
                                            MatrixType>::AdditionalData &
  ...
  get_color_sequence(const bool transpose) const
{
  ...
  if (revert_colors)
    std::reverse(color_sequence.begin(),
                 color_sequence.end()); // n_colors-1, ..., 1, 0

  // *** additionally traverse colors backwards (avoiding duplication of
  // adjacent colors)
...
inline std::shared_ptr<const SubdomainHandler<
  dim,
  typename SchwarzPreconditioner<dim, OperatorType, VectorType, MatrixType>::
    value_type>>
SchwarzPreconditioner<dim, OperatorType, VectorType, MatrixType>::
  get_subdomain_handler() const
{
